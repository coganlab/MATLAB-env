...
/*----------------------------------------------------------------------
%
% Y = NSTREAM_GETRAW_COMEDI_DECIMATE(N, D)
%
% NSTREAM_GETRAW_COMEDI_DECIMATE returns N data points back from last written position 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  plhs[0] = mxCreateDoubleMatrix(NSTREAM_NUM_COMEDI_CHANNELS,number_of_samples, mxREAL);
  ...
  for(i=0; i < decimated_number_of_samples*NSTREAM_NUM_COMEDI_CHANNELS; i++)
  {  
     raw_matrix[NSTREAM_NUM_COMEDI_CHANNELS*decimated_number_of_samples - 1 - i] = share->comedi_buffer[write_pos];
     write_pos--; 	
