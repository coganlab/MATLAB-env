...
/*----------------------------------------------------------------------
%
% Y = NSTREAM_GETRAWINT_COMEDI_DECIMATE(T1,T2,D)
%
% NSTREAM_GETRAWINT_COMEDI_DECIMATE returns data for a given time interval from the shared memory buffer
%
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  unsigned long comedi_buf_time = share->comedi_time;

  /* verify that the range requested is available in the buffer */
  if (comedi_buf_time < to_samp)
      mexErrMsgTxt("T2 > comedi_time");

  if (from_samp < comedi_buf_time - (NSTREAM_DURATION * NSTREAM_SR))
    if (comedi_buf_time > comedi_buf_time - (NSTREAM_DURATION * NSTREAM_SR)) 
      mexErrMsgTxt("T1 < comedi_time - buffer length in samples");

  /* ok, we're lookin good.  create the return matrix and do the copy! */
  
  /* Create matrix for the return argument. [mxREAL|mxCOMPLEX] */
  plhs[0] = mxCreateDoubleMatrix(NSTREAM_NUM_COMEDI_CHANNELS, dur_samp, mxREAL);
  return_matrix = mxGetPr(plhs[0]); 

  /*========================== Critical Section =======================*/

  /*writing backwards into the matrix*/
  int write_pos = ((to_samp*NSTREAM_NUM_COMEDI_CHANNELS)%(NSTREAM_COMEDI_NUM_SAMPLES));
  if (write_pos == 0) { write_pos = NSTREAM_COMEDI_NUM_SAMPLES; }
  write_pos--;
 
  for(i=0; i < dur_samp*NSTREAM_NUM_COMEDI_CHANNELS; i++)
  {  
     return_matrix[NSTREAM_NUM_COMEDI_CHANNELS*dur_samp - 1 - i] = share->comedi_buffer[write_pos];
     write_pos--;

     if (((i+1)%NSTREAM_NUM_COMEDI_CHANNELS) == 0)
     {
        write_pos -= NSTREAM_NUM_COMEDI_CHANNELS*(decimation_factor-1);
     }
      
     if(write_pos < 0) { 	 
	   write_pos = NSTREAM_COMEDI_NUM_SAMPLES - (write_pos*-1); 
     }
  }  
