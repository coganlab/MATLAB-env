...
function ntools_procComediVideoSync(recording_filename_root)
    ...
    CH = experiment.recording.comedi.num_channels;
    FS = experiment.recording.comedi.sample_rate;
    T = 0.25;
    SAMPLE_SIZE = experiment.recording.comedi.sample_size;
    THRESHOLD = 1e3;
...
%            disp(['COMEDI: Loop ' num2str(N)]);
            comedi = fread(comedi_fid, FS*T, [comedi_sample_format '=>single'], ...
                SAMPLE_SIZE*(DECIMATION_FACTOR*CH-1));
