...
if strcmp(sph,'both')
    ...
    cname=(['/mnt/raid/NYUMC/LHSUBJ_130316/label/lh.aparc.a2009s.annot']);
    [vertices label colortable]=read_annotation(cname);
    % get labels 
    % PROD
    prem1=find(label==15733781);
    prem2=find(label==13112341);
    postg=find(label==9221140);
    preg=find(label==11832380);
    subg=find(label==14423103);
    ...
    col2(postg,:)=repmat([0 0 1], [length(postg) 1]);
    col2(preg,:)=repmat([0 0 1], [length(preg) 1]);
    col2(subg,:)=repmat([0 0 1], [length(subg) 1]);
    ...
    hold on;
    trisurf(sub.tri, sub.vert(:,1), sub.vert(:, 2), sub.vert(:, 3),  'FaceVertexCData', col2,'FaceColor', 'interp','FaceAlpha',0.5);
     hold on;
    trisurf(sub.tri, sub.vert(:,1), sub.vert(:, 2), sub.vert(:, 3),  'FaceVertexCData', col3,'FaceColor', 'interp','FaceAlpha',0.5);
end
