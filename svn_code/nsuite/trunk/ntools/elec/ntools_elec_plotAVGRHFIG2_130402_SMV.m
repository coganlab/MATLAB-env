 ...
 %   [FileName,PathName] = uigetfile('*.txt','Select the electrodes text file','/home/ccarlson/loc/');
  %  [surfname, surfpath] = uigetfile('*.mat','Select the patient brain surf',PathName,'MultiSelect','on');
   % surf = strcat(surfpath,surfname);    
    surf = '/mnt/raid/nim1_backup/nim1-raid-backup/NYUMC/RHSUBJ_130316/loc/RH.AVG.rh.pial.mat';
% elseif nargin==2
...
% end
load('/mnt/raid/nim1_backup/nim1-raid-backup/NYUMC/RHSUBJ_130316/ALLELECS_RH.mat');
...
Alist=[1,5,8,12,13,14,15,16,17,19,22,23,25,26,27];
SMVlist=setdiff(1:31,Alist);


for i=1:length(Alist) % AUD
      hold on;
    plot3(elec(Alist(i),1),elec(Alist(i),2),elec(Alist(i),3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
    hold on;
    plot3(elec(Alist(i),1),elec(Alist(i),2),elec(Alist(i),3),['g' 'o'],'MarkerFaceColor','g','MarkerSize',marksize);
end


for i=1:length(SMVlist) % AUD
      hold on;
      elec(SMVlist(i),1)=elec(SMVlist(i),1)+15;
    plot3(elec(SMVlist(i),1),elec(SMVlist(i),2),elec(SMVlist(i),3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
    hold on;
    plot3(elec(SMVlist(i),1),elec(SMVlist(i),2),elec(SMVlist(i),3),['m' 'o'],'MarkerFaceColor','m','MarkerSize',marksize);
end
...
for i=95:97 % SM
      hold on;
    plot3((elec(i,1)+15),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
    hold on;     
    plot3((elec(i,1)+15),elec(i,2),elec(i,3),['r' 'o'],'MarkerFaceColor','r','MarkerSize',marksize); % SM
end
...
%   if i==105 || i==106 || i==107 || i==110 || i==111 || i==112 || i==113 
    ...
    hold on;
    plot3((elec(i,1)-15),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblackSMA);
      hold on
         plot3((elec(i,1)+15),elec(i,2),elec(i,3),['g' 'o'],'MarkerFaceColor','r','MarkerSize',marksize,'LineWidth',13);
   end
%    
 end
