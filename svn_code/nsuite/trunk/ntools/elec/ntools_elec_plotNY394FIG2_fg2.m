...
elec2(:,1)=elec2(:,1)+25; % this is the adjustment for the RH, standard is +25
...
elseif plt==2 && ~isempty(elec_cell)
    showpart = 'S';
    nyu_plot(surf_brain,sph,elec2,char(elec_cell(:,1)),'w',labelshow);
...
markblackdelay=20;
for i=1:length(elec)
        if i==8 || i==14 || i==15 || i==20 || i==21 || i==22 || i==24 || i==30 || i==32 || i==63 % AUD [8 14 15 20 21 22 24 30 32 53];
        plot3(elec(i,1),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
        hold on;
        if i==32 || i==63
            plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor','k','MarkerSize',markblackdelay);
    ...
    elseif i==27 || i==29 || i==34 || i==36 || i==38 || i==42 || i==44 || i==55 % PROD [27 29 34 36 38 40 42 49]
        plot3(elec(i,1),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
        hold on;
        if i==44
            plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor','k','MarkerSize',markblackdelay);
            plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor',[0.1 0.1 1],'MarkerSize',marksize);
   ...
   %     display(i) % removed bad channels
       elseif i==37 || i==47 || i==53 || i==54  % SM 37 43 47 48
           %elec(i,1)=elec(i,1)-5; % move more right!
       plot3(elec(i,1),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblack);
        hold on;
        if i==37 || i==53
            plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor','k','MarkerSize',markblackdelay);
            plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor','r','MarkerSize',marksize);
            
        else
        plot3(elec(i,1),elec(i,2),elec(i,3),['r' 'o'],'MarkerFaceColor','r','MarkerSize',marksize);
        end
     elseif i==43 || i==50 || i==59  % DEL 39 41 46
        plot3(elec(i,1),elec(i,2),elec(i,3),['k' 'o'],'MarkerFaceColor','k','MarkerSize',markblackdelay);
        hold on;
        plot3(elec(i,1),elec(i,2),elec(i,3),['y' 'o'],'MarkerFaceColor','y','MarkerSize',marksize);
        %plot3(elec(i,1),elec(i,2),elec(i,3),['r' 'o'],'MarkerFaceColor','r','MarkerSize',marksize,'LineWidth',7);
    elseif 0%ismember(i,misschans)
        display(i)
    else
    %plot3(elec(i,1),elec(i,2),elec(i,3),[color 'o'],'MarkerFaceColor',color,'MarkerSize',marksize);
     plot3(elec(i,1),elec(i,2),elec(i,3),'o','MarkerEdgeColor',[0.9 0.9 0.9],'MarkerFaceColor',[0.9 0.9 0.9],'MarkerSize',10);
    end
    if label==1
