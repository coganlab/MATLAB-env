...
if strcmp(sph,'both')
    ...
    labelfile='/mnt/raid/NYUMC/RHSUBJ_130316/label/rh.aparc.a2009s.annot';
   ...
   % ORBcol=[0/255 191/255 255/255]; % deep sky blue
    SUPRAcol=[205/255 92/255 92/255]; % indian red
    PLANcol=[152/255 251/255 152/255]; % pale green 
   % INScol=[0/255 191/255 255/255]; % deep sky blue
   % % D-K Atlas
    ...
    % Pars orbitalis label 20, 3302420
    % Supramarginal label 32, 1351760
    
    % STG
    STGidx=find(label==14433500);
    TRIidx=find(label==9231540);
    OPERidx=find(label==6559840);
    PREidx=find(label==11832380);
    POSTidx=find(label==9221140);
    SUPRAidx=find(label==3957860);
    PLANidx=find(label==1346585);
...
%    % ORBidx=find(label==3302420);
%      INSidx=find(label==2146559);
%     SUPRAidx=find(label==1351760);
   ...
   % col(ORBidx,:)=repmat(ORBcol(:)',[size(ORBidx,1) 1]);
  %  col(INSidx,:)=repmat(INScol(:)',[size(INSidx,1) 1]);
    col(SUPRAidx,:)=repmat(SUPRAcol(:)',[size(SUPRAidx,1) 1]); 
    col(PLANidx,:)=repmat(PLANcol(:)',[size(PLANidx,1) 1]); 
