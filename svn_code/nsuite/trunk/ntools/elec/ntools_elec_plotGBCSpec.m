function ntools_elec_plotGBCSpec(sub2,hemi,labels)
...
% default labels = [5 13 14 27 29 30 35 37 69 70 71 75];
...
if plt==1 && ~isempty(elec_grid)
    ...
    %nyu_plot2(surf_brain,sph,cell2mat(elec_grid(:,2:4)),char(elec_grid(:,1)),'r',labelshow,1,11.3,sub2,hemi);
     nyu_plot2(surf_brain,sph,elec_grid2,char(elec_grid(:,1)),'k',labelshow,1,28,sub2,hemi,labels);
...
%         'FaceVertexCData', col2,'FaceAlpha',alpha);
    ...
    %col=load([NYUMCDIR '/NY226/surf/colormatrix.mat']);
     colormatrix=colorgen2(sub2,hemi,labels);
    col=colormatrix;
...
for i=1:length(elec)
    ...
    if label==1
        text('Position',[elec(i,1) elec(i,2) elec(i,3)],'String',elecname(i,:),'Color','w', 'FontSize', 28);
    end
end
set(light,'Position',[-1 0 1]); 
    if strcmp(sph,'lh')
...
%% colorgen2
function colormatrix = colorgen2(sub2,hemi,labels)
nyumc;
