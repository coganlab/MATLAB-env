...
if strcmp(sph,'both')
  ...
  %  col2(stgA,:)=repmat([51/255 25/255 0], [length(stgA) 1]);
    ...
    subA=idx(1:floor(0.70*length(tmp)));
    subB=idx((ceil(0.70*length(tmp)+1)):end);
    
    %subA=idx(1:floor(length(tmp)/2));
    %subB=idx(ceil(length(tmp)/2)+1:end);
    
    subAnt=subg(subA);
    subPost=subg(subB);
  %  stgA=stg1(Alab); % 13
  %  stgP=stg1(Plab); % 14
    col2(subAnt,:)=repmat([1 0 0], [length(subAnt) 1]);
    col2(subPost,:)=repmat([0 0 1], [length(subPost) 1]);
