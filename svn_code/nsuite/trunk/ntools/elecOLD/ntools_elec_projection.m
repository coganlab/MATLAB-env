...
[surf_v surf_f] = read_surf([subjectpath '/surf/' sph '.pial-outer-smoothed']);
...
mesial = find((abs(x)-abs(surf.coords(:,1)))>0);
...
zmin = min(z)-5;
zmax = max(z)+5;
a = find(brain(:,2)>ymin & brain(:,2)<ymax & brain(:,3)>zmin & brain(:,3)<zmax);
elec_area = brain(a,:);
...
vec1 = repmat(ctr',[length(elec) 1])-elec(:,2:4);
vec2 = repmat(ctr',[length(elec_area) 1])-elec_area;
for j=1:length(vec1)
