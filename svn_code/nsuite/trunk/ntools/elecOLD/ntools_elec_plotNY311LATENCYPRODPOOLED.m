...
nyumc;
load([NYUMCDIR '/NY311/loc/NY311enls.mat']);
ienl=find(enl>0);
shading interp;
lighting gouraud;
material dull;
...
%          plot3(elec(i,1),elec(i,2),elec(i,3),'o','MarkerEdgeColor',[0.9 0.9 0.9],'MarkerFaceColor',[0.9 0.9 0.9],'MarkerSize',40);
    ...
    if label==1
        if intersect(i,ienl)>0
        text('Position',[elec(i,1) elec(i,2) elec(i,3)],'String',enl(i),'Color','k','FontSize',24);
         text('Position',[elec(i,1)+7 (elec(i,2)+4) elec(i,3)],'String',enl2(i),'Color','k','FontSize',16);
        end
    end
end
set(light,'Position',[-1 0 1]); 
