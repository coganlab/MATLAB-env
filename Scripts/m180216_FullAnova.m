...
for iChan=1:length(AnalParams.Channel)
    ...
    for iCond=1:8
      ...
      %  data=cat(1,data,Auditory_Spec{iCond}{iChan});
        data=cat(1,data,Auditory_Spec{iCond}{iChan}./repmat(mean(Auditory_Spec{iCond}{iChan}(:,1:10,:),2),1,size(Auditory_Spec{iCond}{iChan},2),1));
    end
  ...
  %  FS=cat(2,F1,F2,F3);
    FS=cat(2,FS,ones(size(FS,1),1));
    data=sq(mean(data(:,:,fRange),3));
  %  data=(data-data_m)./data_s;
    tRange=1:size(Auditory_Spec{1}{1},2);
    ...
    for iTRange=1:length(tRange)
      ...
      [b,bint,r,rint,stats] = regress(dataT,FS);
        % [P T]=anovan(dataT,FS,'model','full','display','off');
        %         for iT=1:7;
        ...
        for iPerm=1:1000;
            [b,bint,r,rint,stats] = regress(dataT(shuffle(1:size(dataT,1)),:),FS);
            Tvals_S(iChan,iTRange,iPerm,:)=b; %Tvals1;
        end
      %  ModelVals{iChan}{iTRange}=mdl;
      %  Pvals(iChan,iTRange,:)=stats(3);
      %  Rvals(iChan,iTRange,:)=stats(1);
    end
    display(iChan)
    
end
figure;for A=1:6;subplot(2,3,A);imagesc(sq(Tvals(:,:,A)));caxis([0 0.2]);title(num2str(A));end;
figure;for A=1:6;subplot(2,3,A);imagesc(sq(-Tvals(:,:,A)));caxis([0 0.2]);title(num2str(A));end;
