...
for SN = [9:12,14,15:17];
    ...
    for iChan=1:length(AnalParams.Channel);
        ...
        for iCond=1:8;
            ...
            ATrials2=ATrials1{iCond}(tmpIDXA);
            MTrials2=MTrials1{iCond}(tmpIDXM);
            STrials2=STrials1{iCond}(tmpIDXS);
...
for iChan=1:size(Spec_Chan_All_Aud,2);
    ...
    for iTime=1:40;
        ...
        actdiffA=mean(tmpA(:,iTime))-mean(mean(tmpA(:,1:10)));
        combvalA=cat(1,mean(tmpA(:,iTime),2),reshape(tmpA(:,1:10),size(tmpA,1)*10,1));
        %     actdiffRA=mean(tmpRA(:,iTime))-mean(mean(tmpRA(:,1:10)));
        %     combvalRA=cat(1,mean(tmpRA(:,iTime),2),reshape(tmpRA(:,1:10),size(tmpRA,1)*10,1));
        
        actdiffP=mean(tmpP(:,iTime))-mean(mean(tmpA(:,1:10)));
        combvalP=cat(1,mean(tmpP(:,iTime),2),reshape(tmpA(:,1:10),size(tmpA,1)*10,1));
        ...
        pvalsA(iChan,iTime)=length(find(permvalA>actdiffA))./nPerm;
        pvalsP(iChan,iTime)=length(find(permvalP>actdiffP))./nPerm;
    end
    display(iChan)
end
...
sigVal=0.05;
ClusterSize=6;
pvalsAM=zeros(size(pvalsA,1),size(pvalsA,2));
iiS=find(pvalsA<sigVal);
pvalsAM(iiS)=1;




pvalsPM=zeros(size(pvalsP,1),size(pvalsP,2));
iiS=find(pvalsP<sigVal);
pvalsPM(iiS)=1;
...
save('G:\D_Data\misc\Pvals_AUD_PROD_HG_70120_Outlier8_TimePtsvsBaseExp_RespAlignedAligned_WrongRespExc_IndivBaseSub_D22andD23InclFixed_MoreTrialsCollapsed_1025_.mat','pvals*','ii*','Sig*');
