...
%         permvalRP(iPerm)=mean(combvalRP(sIdxRP(1:size(tmpRP,1))))-mean(combvalRP(sIdxRP(size(tmpRP,1)+1:length(sIdxRP))));    
    ...
    pvalsWNWA(iChan,iTime)=length(find(permvalWA>actdiffWA))./nPerm;
%     pvalsRA(iChan,iTime)=length(find(permvalRA>actdiffRA))./nPerm;
    pvalsWNWP(iChan,iTime)=length(find(permvalWP>actdiffWP))./nPerm;
...
sigVal=0.025;
ClusterSize=3;
pvalsWNWAM=zeros(size(pvalsWNWA,1),size(pvalsWNWA,2));
iiS=find(pvalsWNWA<sigVal);
pvalsWNWAM(iiS)=1;

pvalsNWWAM=zeros(size(pvalsWNWA,1),size(pvalsWNWA,2));
iiS=find(1-pvalsWNWA<sigVal);
pvalsNWWAM(iiS)=1;


pvalsWNWPM=zeros(size(pvalsWNWP,1),size(pvalsWNWP,2));
iiS=find(pvalsWNWP<sigVal);
pvalsWNWPM(iiS)=1;

pvalsNWWPM=zeros(size(pvalsWNWP,1),size(pvalsWNWP,2));
iiS=find(1-pvalsWNWP<sigVal);
pvalsNWWPM(iiS)=1;
% 
...
clusterStartWNWA=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartWNWA2=zeros(size(Spec_Chan_All_Aud,2),1);

for iChan=1:size(pvalsWNWAM,1);

    tmp=bwconncomp(sq(pvalsWNWAM(iChan,:)));
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ...
            clusterStartWNWA2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartWNWA(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartWNWA(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartWNWA(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartWNWA(iChan,4)=1;
...
%                 clusterStartDA(iChan,6)=1;
    ...
    SigClusterSizeWNWA(iChan)=max(ii2);
    else
        SigClusterSizeWNWA(iChan)=0;
    end
    clear ii2
end

iiWNWA=find(SigClusterSizeWNWA>=ClusterSize);

clusterStartNWWA=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartNWWA2=zeros(size(Spec_Chan_All_Aud,2),1);

for iChan=1:size(pvalsNWWAM,1);

    tmp=bwconncomp(sq(pvalsNWWAM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ...
            clusterStartNWWA2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartNWWA(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartNWWA(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartNWWA(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartNWWA(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartRA(iChan,6)=1;
    ...
    SigClusterSizeNWWA(iChan)=max(ii2);
    else
        SigClusterSizeNWWA(iChan)=0;
    end
    clear ii2
end

iiNWWA=find(SigClusterSizeNWWA>=ClusterSize);


clusterStartWNWP=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartWNWP2=zeros(size(Spec_Chan_All_Aud,2),1);
for iChan=1:size(pvalsWNWPM,1);

    tmp=bwconncomp(sq(pvalsWNWPM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ii3(ii)=tmp.PixelIdxList{ii2B(ii)}(1);
            clusterStartWNWP2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartWNWP(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartWNWP(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartWNWP(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartWNWP(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartDP(iChan,6)=1;
    ...
    SigClusterSizeWNWP(iChan)=max(ii2);
    else
        SigClusterSizeWNWP(iChan)=0;
    end
    clear ii2
end

iiWNWP=find(SigClusterSizeWNWP>=ClusterSize);

clusterStartNWWP=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartNWWP2=zeros(size(Spec_Chan_All_Aud,2),1);
for iChan=1:size(pvalsNWWPM,1);

    tmp=bwconncomp(sq(pvalsNWWPM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ii3(ii)=tmp.PixelIdxList{ii2B(ii)}(1);
            clusterStartNWWP2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartNWWP(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartNWWP(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartNWWP(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartNWWP(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartRP(iChan,6)=1;
    ...
    SigClusterSizeNWWP(iChan)=max(ii2);
    else
        SigClusterSizeNWWP(iChan)=0;
    end
    clear ii2
end

iiNWWP=find(SigClusterSizeNWWP>=ClusterSize);
...
save('G:\D_Data\misc\Pvals_AUD_PROD_HG_70120_Outlier8_WordvsNonExp_RespAlignedAligned_WrongRespExc_IndivBaseSub_D22andD23InclFixed_MoreTrials.mat','pvals*','ii*','Sig*');
