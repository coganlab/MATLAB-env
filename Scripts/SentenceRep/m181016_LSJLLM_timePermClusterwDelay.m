...
AnalParams.ArtifactThreshold = 12; %8 %12;
 ...
 %   CondParams.bn = [0,2000];
  ...
  %  fRange=30:50;
  clear Spec_Chan_All_Prod Spec_Chan_All_Aud Spec_Chan_All_Delay Spec_Chan_All_Start
    outcut=10;
...
for iChan=1:size(Spec_Chan_All_Aud,2);
   ...
   clear tmpIDXD
          ...
          %  tmp=tmp-mean(mean(tmp));
            [m s]=normfit(tmpS(:));
            [i j]=find(tmpS>(outcut*s+m));
            tmpIDXS=setdiff(1:size(tmpS,1),unique(i));
         ...
         dataS(iChan,iCond,:)=mean(Spec_Chan_All_Start{iChan}{iCond}(tmpIDXS,:))./mean(mean(Spec_Chan_All_Start{iChan}{iCond}(tmpIDXS,1:10)));     
    end
end
...
for iChan=1:size(Spec_Chan_All_Aud,2);
    ...
    for iCond=1:4;
        ...
        tmpJLD=cat(1,tmpJLD,Spec_Chan_All_Delay{iChan}{iCond+7});
        tmpLMD=cat(1,tmpLMD,Spec_Chan_All_Delay{iChan}{iCond+14});
        tmpLSS=cat(1,tmpLSS,Spec_Chan_All_Start{iChan}{iCond});
        tmpJLS=cat(1,tmpJLS,Spec_Chan_All_Start{iChan}{iCond+7});
        tmpLMS=cat(1,tmpLMS,Spec_Chan_All_Start{iChan}{iCond+14});       
    end
    
     %   bSig=cat(1,tmpLSA(:,1:10),tmpLMA(:,1:10),tmpJLA(:,1:10));
     bSig=repmat(cat(1,tmpLSA(:,1:10),tmpLMA(:,1:10),tmpJLA(:,1:10)),1,4);
  %   bSigD=repmat(cat(1,tmpLSS(:,1:10),tmpLMS(:,1:10),tmpJLS(:,1:10)),1,2);
     bSigD=repmat(cat(1,tmpLSA(:,1:10),tmpLMA(:,1:10),tmpJLA(:,1:10)),1,2);
     bSigS=repmat(cat(1,tmpLSS(:,1:10),tmpLMS(:,1:10),tmpJLS(:,1:10)),1,2);

     aSig=tmpLSA;
     [zValsRawAct pValsRawAct actClustLSA{iChan}]=timePermCluster(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpLMA;
     [zValsRawAct pValsRawAct actClustLMA{iChan}]=timePermCluster(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpJLA;
     ...
     aSig=tmpLMP;
     [zValsRawAct pValsRawAct actClustLMP{iChan}]=timePermCluster(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpJLP;
     [zValsRawAct pValsRawAct actClustJLP{iChan}]=timePermCluster(aSig,bSig,nPerm,numTails,zThresh);
      aSig=tmpLSD;
     [zValsRawAct pValsRawAct actClustLSD{iChan}]=timePermCluster(aSig,bSigD,nPerm,numTails,zThresh);
     aSig=tmpLMD;
     [zValsRawAct pValsRawAct actClustLMD{iChan}]=timePermCluster(aSig,bSigD,nPerm,numTails,zThresh);
     aSig=tmpJLD;
     [zValsRawAct pValsRawAct actClustJLD{iChan}]=timePermCluster(aSig,bSigD,nPerm,numTails,zThresh);
     aSig=tmpLSS;
     [zValsRawAct pValsRawAct actClustLSS{iChan}]=timePermCluster(aSig,bSigS,nPerm,numTails,zThresh);
     aSig=tmpLMS;
     [zValsRawAct pValsRawAct actClustLMS{iChan}]=timePermCluster(aSig,bSigS,nPerm,numTails,zThresh);
     aSig=tmpJLS;
     [zValsRawAct pValsRawAct actClustJLS{iChan}]=timePermCluster(aSig,bSigS,nPerm,numTails,zThresh);     
 display(iChan);
end    
save([DUKEDIR '/SentenceRep/' Subject(SN).Name '_LocalizerLSLMHL_70100_Outlier8_timepointsvsbaselineClusterZ_StartDelay.mat'],'AnalParams','actClust*','data*');
clear actClust*
clear data*
end