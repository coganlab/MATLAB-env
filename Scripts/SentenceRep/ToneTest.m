...
%subject = 'Test2';
%subjecDir = ['C:\Psychtoolbox_Scripts\SentenceRep\' subject];
soundDir = 'C:\Psychtoolbox_Scripts\SentenceRep\';
nBlocks = 1; % 10
trialCount=0;
blockCount=0;
trialEnd=5;
...
[tone500]=audioread([soundDir 'Tone500.wav']);
...
for iB=1:nBlocks;
 ...
 %   [trialStruct,trialShuffle,shuffleIdx,shuffBase] = CreateTrialStructure(rotNumb,nTrials,practice);   
    ...
    delTimeBaseSeconds= 4.5; % 0.5 Base Duration of Cue s
...
%         end
       ...
       sound=tone500;% tone500; %tone500; %sound=eval(trialStruct.sound{trialShuffle(2,iTrials)});
        sound=0.1*sound(:,1);
      %  go=trialStruct.go{trialShuffle(3,iTrials)};
       ...
       % end
        ...
        soundTimeSecs = length(sound)./freqS; %2000; %freqS; %max(cat(1,length(kig),length(pob)))./freqS;
...
%         end
   ...
   %     save([subjecDir '\' subject '_Block_' num2str(blockCount+1) fileSuff '_TrialData.mat'],'trialInfo','trialStruct','trialShuffle')
    ...
    date1=date;
    filename = (['ToneTest_' date1 '_AllTrials.wav']);
    audiowrite([filename],audiodata,freqR);
    PsychPortAudio('Stop', pahandle2);
  %  PsychPortAudio('Close', pahandle2);
    % Stop playback
