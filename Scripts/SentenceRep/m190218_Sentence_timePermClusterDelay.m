...
%             
                        ...
                        % Spec_Chan_All_Start{iChan2}{iCond}=sq(mean(Start_Spec{iCond}{iChan}(STrialsIdx,:,fRange),3));             
             Spec_Chan_All_Delay{iChan}{iCond}=sq(mean(Delay_Spec{iCond}{iChan}(tmpIDXD,:,fRange),3));
%            Spec_Chan_All_Prod{iChan2}{iCond}=sq(mean(Motor_Spec{iCond}{iChan}(MTrialsIdx,:,fRange),3))./sq(mean(mean(Auditory_Spec{iCond}{iChan}(ATrialsIdx,1:10,fRange),2),3));

            Spec_Subject_Delay(iChan)=SN;
%             if iCond<=4
...
save([DUKEDIR '/SentenceRep/' Subject(SN).Name '_Sentence_70100_Outlier8_timepointsvsbaselineClusterZDelay.mat'],'AnalParams','actClust*','dataA','dataP','dataD');
clear actClust*
clear data*
end
