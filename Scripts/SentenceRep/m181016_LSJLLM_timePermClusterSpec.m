...
for iChan=1:size(Spec_Chan_All_Aud,2);
    ...
    tmpLMP=[];
    for iCond=1:4;
        tmpLSA=cat(1,tmpLSA,Auditory_Spec{iCond}{iChan});
        tmpJLA=cat(1,tmpJLA,Auditory_Spec{iCond+7}{iChan});
        tmpLMA=cat(1,tmpLMA,Auditory_Spec{iCond+14}{iChan});
        tmpLSP=cat(1,tmpLSP,Motor_Spec{iCond}{iChan});
        tmpJLP=cat(1,tmpJLP,Motor_Spec{iCond+7}{iChan});
        tmpLMP=cat(1,tmpLMP,Motor_Spec{iCond+14}{iChan});
    end
   ...
   %  bSig=repmat(cat(1,mean(tmpLSA(:,1:10,1:200),2),mean(tmpLMA(:,1:10,1:200),2),mean(tmpJLA(:,1:10,1:200),2)),1,40);
     ...
     aSig=tmpLSA(:,:,:);
     [zValsRawActLSA{iChan} pValsRawAct actClustLSA{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpLMA;
     [zValsRawActLMA{iChan} pValsRawAct actClustLMA{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpJLA;
     [zValsRawActJLA{iChan} pValsRawAct actClustJLA{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpLSP;
     [zValsRawActLSP{iChan} pValsRawAct actClustLSP{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpLMP;
     [zValsRawActLMP{iChan} pValsRawAct actClustLMP{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
     aSig=tmpJLP;
     [zValsRawActJLP{iChan} pValsRawAct actClustJLP{iChan}]=timePermClusterSpec(aSig,bSig,nPerm,numTails,zThresh);
 display(iChan);
end    
save([DUKEDIR '/SentenceRep/' Subject(SN).Name '_LocalizerLSLMHL_Outlier8_timepointsvsbaselineSpecClusterZ.mat'],'AnalParams','actClust*','zValsRawAct*');
clear actClust*
end