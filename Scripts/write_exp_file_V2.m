function write_exp_file_V2(subject,subjdate,header,chans)
...
% Note: v2 uses a diffeent edf reader (edfread).
duke;
...
experiment.recording.nspike.sample_rate=header.frequency(1); % 2000
experiment.processing.ieeg.sample_rate=header.frequency(1); %2000;
experiment.processing.ieeg.lowpass=header.frequency(1)/2; %1000;
experiment.processing.ieeg.sample_rate=header.frequency(1); %2000;
experiment.recording.sample_rate=header.frequency(1); %2000;
...
for iChan=1:experiment.recording.nspike_num_channels_to_write_low;
    experiment.channels(iChan).name=header.label{iChan}; %header.channelname(iChan,:);
    experiment.channels(iChan).lowpass_cutoff=header.frequency(1)/2;
end
