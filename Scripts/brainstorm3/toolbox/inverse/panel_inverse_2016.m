function varargout = panel_inverse_2016(varargin)
% PANEL_INVERSE_2016: Inverse modeling GUI.
%
% USAGE:  bstPanel = panel_inverse_2016('CreatePanel', sProcess, sFiles)                                                 : Called from the pipeline editor
%         bstPanel = panel_inverse_2016('CreatePanel', Modalities, isShared, HeadModelType, nSamplesNoise, nSamplesData) : Called from the interactive interface
...
function [bstPanelNew, panelName] = CreatePanel(Modalities, isShared, HeadModelType, nSamplesNoise, nSamplesData) %#ok<DEFNU>
    ...
    % GUI CALL:  panel_inverse_2016('CreatePanel', Modalities, isShared, HeadModelType, nSamplesNoise, nSamplesData)
    if iscell(Modalities)
        isProcess = 0;
        isFull = 0;
        % Get default inverse options
        OPTIONS = bst_inverse_linear_2016();
    ...
    % PROCESS CALL:  panel_inverse_2016('CreatePanel', sProcess, sFiles)
    else
        isProcess = 1;
        % Get inputs
        sProcess = Modalities;
...
%  =================================================================================
    ...
    function UpdatePanel(isForced, isFirstCall)
        ...
        Comment = [Comment, ': ', process_inverse_2016('GetModalityComment', selModalities)];
