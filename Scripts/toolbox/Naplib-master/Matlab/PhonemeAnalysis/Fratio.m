function [f,fs,Ltmp] = Fratio(D,L,Electrode,Attribute,phnsub)
...
elseif strcmp(Attribute,'manner')
    ...
    for cnt1 = 1:length(phnsub)
        ...
        for cnt2 = 1:length(atlist)
            if ~isempty(find(strcmpi(atr,atlist{cnt2}), 1)), atvec(:,cnt2)=1;end
        end
        AL(:,L==cnt1)=repmat(atvec',[1 length(find(L==cnt1))]);
    end
    Ltmp=zeros(1,length(AL));
    ind=ismember(phnsub,attribute2phoneme('plosive'));
    %Ltmp(1,find(ismember(L,ind)))=1;
    Ltmp(1,ind) = 1;
    ind=ismember(phnsub,attribute2phoneme('fricative'));
    Ltmp(1,ind) = 2;
   % Ltmp(1,find(ismember(L,ind)))=2;
    ind=ismember(phnsub,attribute2phoneme('nasal'));
    Ltmp(1,ind) = 3;
   % Ltmp(1,find(ismember(L,ind)))=3;
    ind=ismember(phnsub,attribute2phoneme('syllabic'));
    Ltmp(1,ind) = 4;
...
if strcmp(Electrode,'all')
    for t=1:size(D,2)
        tmp=squeeze(D(:,t,:));
        [~, f(:,t) ,fs(:,t)] = DPrime(tmp,Ltmp(:),label);
    end
elseif strcmp(Electrode,'individual')
    for t=1:size(D,2)
        tmp=squeeze(D(:,t,:));
        for cnt=1:size(D,1)
            [~, f(cnt,t) ,fs(cnt,t)] = DPrime(tmp(cnt,:),Ltmp(:),label);
        end
    end
end
