  function [filteredData,cfs,sigma_fs,hilbdata]=CUprocessingHilbertTransform_filterbankGUI_optim(d,Fs,freqRange)
...
for f = 1:npbs
    ...
    % Apply filter and compute Hilbert transform in a vectorized manner
    filteredFFTData = fftData .* (H .* h);
    hilbdata = ifft(filteredFFTData, T, 2);
    filteredData(:,:,f) = hilbdata;
end
