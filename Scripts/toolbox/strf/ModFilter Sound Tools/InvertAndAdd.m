function ytrunc = InvertAndAdd(s, increment, winLength, iteration, doFFTShift, initialPhase)
% InvertAndAdd to perform one stage of the Spectrogram Inversion.
%	y = InvertAndAdd(s, increment, winLength, iteration, doFFTShift, initialPhase);
%	Perform one stage of the Spectrogram Inversion.
%	Invert the (complex) spectrogram (s), assuming frames are [increment] samples apart
%	and the window is [winLength] samples long in the time domain.
%	The [iteration] variable controls how phase is estimated (phase guessing is only done
%	on the first iteration.)
%	[doFFTShift] says whether the data was shifted to give cosine phase before FFT'ing.
% 	[initialPhase] determines the phase guessing algorithm used on iteration 1.
...
% In the original code the length of FFT was deduced from the y axis of the
% spectrum - this code does not work with GaussianSpecturm
...
[nfbands frameCount] = size(s);
if rem(winLength, 2) == 1
    winLength = winLength +1;
end
fftLen = winLength;   
if ( (fftLen/2 +1)  ~= nfbands )
    fprintf(1, 'Error in InvertAndAdd: Window Length does not match spectrum resolution');
end
...
% Remove padding added in Gaussian Spectrum
ytrunc = y(winLength/2+1:end-winLength/2);
