...
Trials2 = recodeTrialsAuditorySternberg(Trials);



   % ieegS=trialIEEG(Trials,AnalParams.Channel,'ListenCue',[-1000 1500]); % 9 items + 1 second for 
    ieegA=trialIEEG(Trials2,AnalParams.Channel,'FirstStimAuditory',[-1000 1500]); % 9 items + 1 second for 
...
%     for iChan=1:size(ieegS,2);
%         for iTrials=1:size(ieegS,1);
%              ieegS_DEC(iChan,iTrials,:)=decimate(double(sq(ieegS(iTrials,iChan,:))),4);            
% %           ieegA_DEC(iChan,iTrials,:)=decimate(double(sq(ieegA(iTrials,iChan,:))),4);
%          %   ieegD_DEC(iChan,iTrials,:)=decimate(double(sq(ieegD(iTrials,iChan,:))),4);
%         %    ieegR_DEC(iChan,iTrials,:)=decimate(double(sq(ieegR(iTrials,iChan,:))),4);
...
% srate=2048; 
if srate<2048
% specSW=zeros(size(ieegS_DEC,1),size(ieegS_DEC,2),36,31);
specAW=zeros(size(ieegA_DEC,1),size(ieegA_DEC,2),36,31);
%specDW=zeros(size(ieegD_DEC,1),size(ieegD_DEC,2),34,81);
%specRW=zeros(size(ieegR_DEC,1),size(ieegR_DEC,2),34,81); 
else
%specSW=zeros(size(ieegS_DEC,1),size(ieegS_DEC,2),34,49);
specAW=zeros(size(ieegA_DEC,1),size(ieegA_DEC,2),36,49);
%specDW=zeros(size(ieegD_DEC,1),size(ieegD_DEC,2),34,129);
%specRW=zeros(size(ieegR_DEC,1),size(ieegR_DEC,2),34,129);
end
...
%[waveS,period,scale,coi]=basewave5(tmpS,srate2,2,250,5,0);
[waveA,period,scale,coi]=basewave6(tmpA,srate2,2,250,5,0.5,0.05);
%[waveD,period,scale,coi]=basewave5(tmpD,srate2,2,200,5,0);
%[waveR,period,scale,coi]=basewave5(tmpR,srate2,2,200,5,0);
...
if srate<2048
  %  aSigD=sq(abs(specDW(iChan,:,:,1:25)));
    aSigA=sq(abs(specAW(iChan,:,:,1:30)));
 %   aSigS=sq(abs(specSW(iChan,:,:,1:30)));
 %   aSigR=sq(abs(specRW(iChan,:,:,:))); 
    bSig=repmat(aSigA(:,:,1:10),1,1,3);
else
  %  aSigD=sq(abs(specDW(iChan,:,:,1:64)));
    aSigA=sq(abs(specAW(iChan,:,:,1:48)));
  %  aSigS=sq(abs(specSW(iChan,:,:,1:48)));
  %  aSigR=sq(abs(specRW(iChan,:,:,:))); 
    bSig=repmat(aSigA(:,:,1:16),1,1,3);
end
% aSigD2=zeros(size(aSigD,1),size(aSigD,3),size(aSigD,2));
...
% aSigS2=zeros(size(aSigS,1),size(aSigS,3),size(aSigS,2));
 bSig2=zeros(size(bSig,1),size(bSig,3),size(bSig,2));
    ...
    for iTrials=1:size(aSigA,1);
     tIdx=shuffle(1:size(bSig,3));
     tIdx=1:size(bSig,3);
  %   aSigR2(iTrials,:,:)=sq(aSigR(iTrials,:,:))';     
  %   aSigD2(iTrials,:,:)=sq(aSigD(iTrials,:,:))';
 ...
 %  [zValsRawActD{iChan} pValsRawAct actClustD{iChan}]=timePermClusterSpec(aSigD2,bSig2,nPerm,numTails,zThresh);
  ...
  %  [zValsRawActD{iChan}{iF} pValsRawD{iChan}{iF} actClustD{iChan}{iF}]=timePermCluster(sq(mean(aSigD2(:,:,freqBand{iF}),3)),sq(mean(bSig2(:,1:size(aSigD2,2),freqBand{iF}),3)),nPerm,numTails,zThresh);
...
save([DUKEDIR '/Output/' Subject(SN).Name '_NeighborhoodSternberg_ClusterTime_5FreqBands_2Tails_AuditoryCollapse.mat'],'zValsRaw*','pValsRaw*','actClust*','freqBand','AnalParams','behaviorMat','period');
clear zValsRaw*
clear pValsRaw*
clear actClust*
end
