...
SNList=[23,26,27,28,29,30,31,35];
%Subject(30).Name='D30'; 
%Subject(31).Name='D31';
nPerm=1000;
numTails=2;
...
specAW=zeros(size(ieegA_DEC,1),size(ieegA_DEC,2),34,337);
specDW=zeros(size(ieegD_DEC,1),size(ieegD_DEC,2),34,129);
specRW=zeros(size(ieegR_DEC,1),size(ieegR_DEC,2),34,129);
elseif srate==2000
specSW=zeros(size(ieegS_DEC,1),size(ieegS_DEC,2),34,47);
specAW=zeros(size(ieegA_DEC,1),size(ieegA_DEC,2),34,328);
specDW=zeros(size(ieegD_DEC,1),size(ieegD_DEC,2),34,125);
specRW=zeros(size(ieegR_DEC,1),size(ieegR_DEC,2),34,125);
end
...
idxS=256:16:(length(tmpS)-255); %-0.5 to 10 s, `33 samples = 1 sec
...
[waveS,period,scale,coi]=basewave5(tmpS,srate2,2,200,5,0);
[waveA,period,scale,coi]=basewave5(tmpA,srate2,2,200,5,0);
...
%     aSigD=sq(abs(specDW(iChan,:,:,1:80)));
%     aSigA=sq(abs(specSW(iChan,:,:,:)));
%     bSig=repmat(aSigA(:,:,1:10),1,1,21);
