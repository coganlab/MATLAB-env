function [H, Z, S, psi] = sfactorization_wilsonWavelet(S,freq,Niterations,tol,fb,init,checkflag)
...
%
% This code was originally part of the FieldTrip software package and
% modified by Ryan Shewcraft
...
%   error('sfactorization_wilson: frequency axis is not evenly spaced');
% end

% if freq(1)~=0
%   warning('sfactorization_wilson: when performing non-parametric spectral factorization, the frequency axis should ideally start at 0, zero padding the spectral density'); 
%   dfreq = mean(dfreq);
%   npad  = freq(1)./dfreq;
%   
%   % update the freq axis and keep track of the frequency bins that are
%   % expected in the output
%   selfreq  = (1:numel(freq)) + floor(npad);
%   freq     = [(0:(npad-1))./dfreq freq];
%   S        = cat(3, zeros(size(S,1), size(S,1), floor(npad)), S);  
% else
...
switch init
  ...
  case 'rand'
    ...
    [tmp, dum] = chol(tmp);
  otherwise
    error('initialization method should be either ''chol'' or ''rand''');
end
h = tmp;
