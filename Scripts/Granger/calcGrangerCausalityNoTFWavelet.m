...
function G = calcGrangerCausalityNoTFWavelet(Lfp1ktmp, Lfp2ktmp, nIter,nPerm)
%function G = calcGrangerCausalityNoTFWavelet(Lfp1ktmp, Lfp2ktmp, tapers, sampling_rate, dn, fk, pad, nIter,nPerm)
...
%
%  Lfp1  =  Signal1 in trials x time format x freq
%  Lfp2  =  Signal2 in trials x time format x freq
...
nwin = size(Lfp1k,2);  nfk = size(Lfp1k,3); % K = size(Lfp1k,3);
