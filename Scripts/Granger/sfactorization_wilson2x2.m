...
%        : cmbidx  = Nx2 cell-array listing the channel pairs for the spectral
...
if nargin<5, 
  error('sfactorization_wilson2x2', 'when requesting multiple pairwise spectral decomposition, ''cmbindx'' needs to be specified');
end
...
if ~all(dfreq==dfreq(1))
  error('sfactorization_wilson2x2', 'frequency axis is not evenly spaced');
end

if freq(1)~=0
  warning_once('sfactorization_wilson2x2', 'when performing non-parametric spectral factorization, the frequency axis should ideally start at 0, zero padding the spectral density'); 
  dfreq = mean(dfreq);
  npad  = freq(1)./dfreq;
