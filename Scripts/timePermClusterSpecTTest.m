function [zValsRawAct pValsRaw actClust]=timePermClusterSpecTTest(aSig,bSig,nPerm,numTails,zThresh)
...
tVals=zeros(size(aSig,2),size(aSig,3));
pVals=zeros(size(aSig,2),size(aSig,3));
for iT=1:size(aSig,2);
    for iF=1:size(aSig,3);
        [H P CI STATS]=ttest2(aSig(:,iT,iF),bSig(:,iT,iF));
        tVals(iT,iF)=STATS.tstat;
        pVals(iT,iF)=P;
    end
end

tValsThresh=zeros(size(aSig,2),size(aSig,3));
ii=find(pVals<0.05);
tValsThresh(ii)=1;
%tic
%  actdiff=mean(aSig(:,iTime))-mean(bSig(:,iTime));
...
tValsP=zeros(nPerm,size(permval,2),size(permval,3));
pValsP=zeros(nPerm,size(permval,2),size(permval,3));
...
for iPerm=1:size(permval,1)
 ...
 sIdx=randperm(size(combval,1));
 sigA=combval(sIdx(idx1),:,:);
 sigB=combval(sIdx(idx2),:,:);
...
for iT=1:size(aSig,2);
    for iF=1:size(aSig,3);
        [H P CI STATS]=ttest2(sigA(:,iT,iF),sigB(:,iT,iF));
        tValsP(iPerm,iT,iF)=STATS.tstat;
        pValsP(iPerm,iT,iF)=P;
    end
end
end

toc
tValsThreshP=zeros(nPerm,size(aSig,2),size(aSig,3));
ii=find(pValsP<0.05);
tValsThreshP(ii)=1;
