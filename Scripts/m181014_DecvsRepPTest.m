...
for iChan=1:size(Spec_Chan_All_Aud,2);
    ...
    for iTime=1:40;
    actdiffDA=mean(tmpDA(:,iTime))-mean(tmpRA(:,iTime));
    combvalDA=cat(1,mean(tmpDA(:,iTime),2),mean(tmpRA(:,iTime),2));
%     actdiffRA=mean(tmpRA(:,iTime))-mean(mean(tmpRA(:,1:10)));
%     combvalRA=cat(1,mean(tmpRA(:,iTime),2),reshape(tmpRA(:,1:10),size(tmpRA,1)*10,1));
    
    actdiffDP=mean(tmpDP(:,iTime))-mean(tmpRP(:,iTime));
    combvalDP=cat(1,mean(tmpDP(:,iTime),2),mean(tmpRP(:,iTime),2));
...
%         permvalRP(iPerm)=mean(combvalRP(sIdxRP(1:size(tmpRP,1))))-mean(combvalRP(sIdxRP(size(tmpRP,1)+1:length(sIdxRP))));    
    ...
    pvalsDRA(iChan,iTime)=length(find(permvalDA>actdiffDA))./nPerm;
%     pvalsRA(iChan,iTime)=length(find(permvalRA>actdiffRA))./nPerm;
    pvalsDRP(iChan,iTime)=length(find(permvalDP>actdiffDP))./nPerm;
...
sigVal=0.025;
ClusterSize=5;
pvalsDRAM=zeros(size(pvalsDRA,1),size(pvalsDRA,2));
iiS=find(pvalsDRA<sigVal);
pvalsDRAM(iiS)=1;

pvalsRDAM=zeros(size(pvalsDRA,1),size(pvalsDRA,2));
iiS=find(1-pvalsDRA<sigVal);
pvalsRDAM(iiS)=1;


pvalsDRPM=zeros(size(pvalsDRP,1),size(pvalsDRP,2));
iiS=find(pvalsDRP<sigVal);
pvalsDRPM(iiS)=1;

pvalsRDPM=zeros(size(pvalsDRP,1),size(pvalsDRP,2));
iiS=find(1-pvalsDRP<sigVal);
pvalsRDPM(iiS)=1;
% 
...
% %
clusterStartDRA=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartDRA2=zeros(size(Spec_Chan_All_Aud,2),1);

for iChan=1:size(pvalsDRAM,1);

    tmp=bwconncomp(sq(pvalsDRAM(iChan,:)));
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ...
            clusterStartDRA2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartDRA(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartDRA(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartDRA(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartDRA(iChan,4)=1;
...
%                 clusterStartDA(iChan,6)=1;
    ...
    SigClusterSizeDRA(iChan)=max(ii2);
    else
        SigClusterSizeDRA(iChan)=0;
    end
    clear ii2
end

iiDRA=find(SigClusterSizeDRA>=ClusterSize);

clusterStartRDA=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartRDA2=zeros(size(Spec_Chan_All_Aud,2),1);

for iChan=1:size(pvalsRDAM,1);

    tmp=bwconncomp(sq(pvalsRDAM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ii3(ii)=tmp.PixelIdxList{ii2B(ii)}(1);
            clusterStartRDA2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartRDA(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartRDA(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartRDA(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartRDA(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartRA(iChan,6)=1;
    ...
    SigClusterSizeRDA(iChan)=max(ii2);
    else
        SigClusterSizeRDA(iChan)=0;
    end
    clear ii2
end

iiRDA=find(SigClusterSizeRDA>=ClusterSize);


clusterStartDRP=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartDRP2=zeros(size(Spec_Chan_All_Aud,2),1);
for iChan=1:size(pvalsDRPM,1);

    tmp=bwconncomp(sq(pvalsDRPM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ii3(ii)=tmp.PixelIdxList{ii2B(ii)}(1);
            clusterStartDRP2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartDRP(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartDRP(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartDRP(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartDRP(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartDP(iChan,6)=1;
    ...
    SigClusterSizeDRP(iChan)=max(ii2);
    else
        SigClusterSizeDRP(iChan)=0;
    end
    clear ii2
end

iiDRP=find(SigClusterSizeDRP>=ClusterSize);

clusterStartRDP=zeros(size(Spec_Chan_All_Aud,2),4);
clusterStartRDP2=zeros(size(Spec_Chan_All_Aud,2),1);
for iChan=1:size(pvalsRDPM,1);

    tmp=bwconncomp(sq(pvalsRDPM(iChan,:)));
    if size(tmp.PixelIdxList,2)>0
    for ii=1:size(tmp.PixelIdxList,2);
        ii2(ii)=size(tmp.PixelIdxList{ii},1);
    end
    ...
    if length(ii2B)>0
        for ii=1:length(ii2B);
            ii3(ii)=tmp.PixelIdxList{ii2B(ii)}(1);
            clusterStartRDP2(iChan)=ii3(1);
            if ii3(ii)>=11 && ii3(ii)<=15
                clusterStartRDP(iChan,1)=1;
            elseif ii3(ii)>=16 && ii3(ii)<=20
                clusterStartRDP(iChan,2)=1;
            elseif ii3(ii)>=21 && ii3(ii)<=25
                clusterStartRDP(iChan,3)=1;
            elseif ii3(ii)>=26 && ii3(ii)<=30
                clusterStartRDP(iChan,4)=1;
%             elseif ii3(ii)>=31 && ii3(ii)<=35
...
%                 clusterStartRP(iChan,6)=1;
    ...
    SigClusterSizeRDP(iChan)=max(ii2);
    else
        SigClusterSizeRDP(iChan)=0;
    end
    clear ii2
end

iiRDP=find(SigClusterSizeRDP>=ClusterSize);
...
save('G:\D_Data\misc\Pvals_AUD_PROD_HG_70120_Outlier8_DecvsRepExp_RespAlignedAligned_WrongRespExc_IndivBaseSub_D22andD23InclFixed_MoreTrialsBC.mat','pvals*','ii*','Sig*');
