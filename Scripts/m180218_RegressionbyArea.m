...
for iLoc=1:8
    ...
    F2=[];
    F3=[];
    ii=find(NewAreaLoc==iLoc);
    AreaTrials=[];
    SubjectTrials=[];
    for iChan=1:length(ii)
        for iCond=1:8
            AreaTrials=cat(1,AreaTrials,Spec_Chan_All{ii(iChan)}{iCond});
            SubjectTrials=cat(1,SubjectTrials,Spec_Subject(ii(iChan))*ones(size(Spec_Chan_All{ii(iChan)}{iCond},1),1));
            F1=cat(1,F1,designMat(1,iCond)*ones(size(Spec_Chan_All{ii(iChan)}{iCond},1),1));
            F2=cat(1,F2,designMat(2,iCond)*ones(size(Spec_Chan_All{ii(iChan)}{iCond},1),1));
            F3=cat(1,F3,designMat(3,iCond)*ones(size(Spec_Chan_All{ii(iChan)}{iCond},1),1));
        end
    end
   ...
   % FS=cat(2,F1,F2,F3,F1.*F2,F1.*F3,F2.*F3); % mains and inter 
    ...
    FS=cat(2,FS,ones(size(FS,1),1)); % add constant
for iTime=1:40
...
%mdl=fitlm(FS,dataT,'interactions','PredictorVars',{'Lex','Task','Phono'},'CategoricalVar',{'Lex','Task','Phono'});
        ...
        Tvals(iLoc,iTime,:)=b; %Tvals1;
        Pvals(iLoc,iTime,:)=stats(3);
        Rvals(iLoc,iTime,:)=stats(1);
        TvalsM{iLoc}{iTime}=mdl;
end
    display(iLoc)
...
for iLoc=1:8;
    for iTime=1:40;
        LexPvals(iLoc,iTime)=TvalsM{iLoc}{iTime}.Coefficients.pValue(2);
        TaskPvals(iLoc,iTime)=TvalsM{iLoc}{iTime}.Coefficients.pValue(3);
        PhonoPvals(iLoc,iTime)=TvalsM{iLoc}{iTime}.Coefficients.pValue(4);
    end
    [pfdr pmask]=fdr(LexPvals(iLoc,:),0.01);
    LexPvalsMask(iLoc,:)=pmask;
    [pfdr pmask]=fdr(TaskPvals(iLoc,:),0.01);
    TaskPvalsMask(iLoc,:)=pmask;
    [pfdr pmask]=fdr(PhonoPvals(iLoc,:),0.01);
    PhonoPvalsMask(iLoc,:)=pmask;
end
