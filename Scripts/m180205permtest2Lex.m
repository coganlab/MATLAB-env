...
AnalParams.fk = 500;
AnalParams.Reference = 'Grand average'; %'Grand average'; %'Grand average'; % other is 'Single-ended';
...
tWin=[11:20];
    ...
    for chan=1:length(AnalParams.Channel)
        ...
        tempycompAUD=sq(mean(mean(tempy(:,tWin,fre),3),2)); % 16-25 for aud, 71:80 for prod, 36:45 for delay?
...
for iChan=1:length(actualvalAUD)
    pvalsAUD(iChan)=length(find(actualvalAUD(iChan)<sq(compvalAUD(iChan,:))))./nPerm;
