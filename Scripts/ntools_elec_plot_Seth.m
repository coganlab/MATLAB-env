...
if exist([PathName, FileName],'file')
    ...
    elec_all = elec_all([1 3 4 5 7]);
    elec_cell = [elec_all{1},num2cell(elec_all{2}),num2cell(elec_all{3}),num2cell(elec_all{4})];
else
    elec_cell = [];
end

% vox = cat(2, elec_all{2}, elec_all{3}, elec_all{4});
% vox2ras = [-1 0 0 128; 0 0 -1 128; 0 -1 0 128; 0 0 0 1];
% ras = (vox2ras *[vox';ones(1,size(vox,1))])';
% for e = 1:size(elec_cell,1)
%     elec_cell{e,2} = ras(e,1);
%     elec_cell{e,3} = ras(e,2);
%     elec_cell{e,4} = ras(e,3);
% end


% for iC=1:length(elec_cell)
%     tmp=cell2num(elec_cell(iC,3));
%     tmp2=cell2num(elec_cell(iC,4));
%     elec_cell(iC,3)=num2cell(tmp+20);
%     elec_cell(iC,4)=num2cell(tmp2-10);
% end
...
elseif plt==3 && ~isempty(elec_depth)
  ...
  %  nyu_plot(surf_brain,sph,cell2mat(elec_depth(:,2:4)),char(elec_depth(:,1)),'g',labelshow,1.5,0.3);
    nyu_plot(surf_brain,sph,cell2mat(elec_depth(:,2:4)),char(elec_depth(:,1)),'g',labelshow,1.5,0.6);
elseif plt==4 && ~isempty(elec_grid) && ~isempty(elec_cell)
    ...
    for i=1:size(elec,1)
        ntools_elec_plotSpheres(elec(i,1),elec(i,2),elec(i,3),2,'b');
        if labelshow==1
...
axis off
hold on;
%cvals=cat(1,[0 0.4470 0.7410],[0.8500 0.3250 0.0980],[0.9290 0.6940 0.1250],[1 0 0],[85/255 107/255 47/255],[1 0 1],[1 105/255 180/255],[128/255 0/255 0/255],[0 100/255 0]);

for i=1:size(elec,1)
  %  color=cvals([floor((i-1)/10)+1],:);
    fprintf('%s %f %f %f\n', elecname(i,1), elec(i,1), elec(i,2), elec(i,3));

    ntools_elec_plotSpheres(elec(i,1),elec(i,2),elec(i,3),radius,color);
