...
if isempty(mgridFname)
    if isempty(bidsDir)
        ...
        if exist(iLocInfoFname,'file') && exist(iLocPairsFname,'file')
           ...
           rawelec = parse_RAS_file(fullfile(RECONDIR,fsSub, 'elec_recon', [fsSub 'PostimpLoc.txt']));
           elecMatrix = rawelec.xyz;
           elecLabels = {};
           for e = 1:numel(rawelec.labels)
               elecLabels{e,1} = [rawelec.hemisphere{e} rawelec.type{e} '_' rawelec.labelprefix{e} num2str(rawelec.labelnumber(e))];
           end
           nElecTotal=size(rawelec.xyz,1);
