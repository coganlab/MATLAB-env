...
for ii = 1:2:numel(varargin)
    switch lower(varargin{ii})
        ...
        case 'machinefmt'
            machinefmt = varargin{ii+1};
        case 'sortlabels'
            sortLabels = varargin{ii+1};
        otherwise
            error('EDFREAD: Unrecognized parameter-value pair specified. Valid values are ''assignToVariables'', ''machinefmt'', and ''targetSignals''.')
    end
end

if numel(machinefmt) > 0
    [fid,msg] = fopen(fname,'r', machinefmt);
else
...
if hdr.records < 0
    fileinfo = dir(fname);
    R = sum(hdr.samples) * 2;
    hdr.records = floor((fileinfo.bytes - hdr.bytes) / R);
end
...
if sortLabels
    labelnums = [];
    for ii = 1:numel(hdr.label)
        if isnumeric(hdr.label(5:end))
            labelnums(ii) = str2double(hdr.label(5:end));
        end
    end
    [~,i] = sort(labelnums);
    hdr.label =  hdr.label(i);
    data = data(i,:);
end    
fclose(fid);