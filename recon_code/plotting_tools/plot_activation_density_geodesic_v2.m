function plot_activation_density_geodesic_v2(subj_labels, activation_values, options)
...
arguments
    ...
    options.dtype = 'pial'; % display brain
...
%     end
    ...
    parfor iElec = 1:size(groupAvgCoords_lh,1)
        ...
        elec_sphere = find(euc_dist_to_each_vert.^2<=pi.*(diskThresh.^2)/4); % select the 20 mm sphere
...
%     end
    ...
    parfor iElec = 1:size(groupAvgCoords_rh,1)
       ...
       elecCoord = groupAvgCoords_rh(iElec,:);
        ...
        elec_sphere = find(euc_dist_to_each_vert.^2<=pi.*(diskThresh.^2)/4); % select the 10 mm sphere
