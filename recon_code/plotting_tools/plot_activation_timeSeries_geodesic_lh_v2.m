function plot_activation_timeSeries_geodesic_lh_v2(subj_labels, activation_values, options)
...
[~, oLayLimits, olayCmapName]=vals2Colormap(activationV_lh(:,1),cLim ,options.colorscale);
toc

%% Plot the brain maps
    
    scrsize = get(0, 'Screensize');
    f = figure('Position', [scrsize(1) scrsize(2) scrsize(3)/3 scrsize(4)/3]);
...
%     
   ...
   for iTime = 1:size(activationV_lh,2)
        if (options.showMesial)   
                ...
                h2 = subplot(1,2,2);% left hemisphere medial
...
%                 camlight_follow(gca);
                ...
                tripatchDG(cort_lh,f,squeeze(overlayData_time_lh(iTime,:,:)));        
                view(270,0)
        ...
        hold on;
    close
    vname = strcat(options.movTitle,'_','left_hemisphere_',stype,...
