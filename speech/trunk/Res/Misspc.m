...
%
%     Usage: [Y,propmiss,failflag] = misspc(X,{usecorr})
%
...
function [X,propmiss,failflag] = misspc(X,usecorr)
  ...
  for iun = 1:length(u_nmiss_obs)       % Cycle thru unique numbers
    if (~failflag)
      ...
      ucombvar = uniquef(combvar,1);        %   of missing vars
      if (ucombvar(1)==0)                   % Ignore cases of no missing vars
        ucombvar = ucombvar(2:end);
      end;
