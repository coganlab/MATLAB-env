% CLUSTER: Produces and bootstraps a UPGMA or neighbor-joining tree, given the 
%          original data matrix (obs x vars) and the name of a function that 
%          will return a symmetric distance matrix.  Bootstraps the characters 
%          within groups or across OTUs.
%
%     Syntax: [dist,topo,support,tot_support] = ...
%                 cluster(X,'distfunc',{iter},{treetype},{outgroup},{bootobs}, ...
%                         {p1},{p2},{p3},{p4},{p5})
%
%          X =          [n x p] data matrix, for which the n taxa (rows) are to be 
%                         clustered.
...
%   10/9/02 -   corrected documentation.

function [distorig,topo,support,tot_supt] = ...
                     cluster(X,func,iter,treetype,outgroup,bootobs,p1,p2,p3,p4,p5)
  ...
  if (bootobs)
    if (~exist('p1'))
      error('  CLUSTER: grouping vector required to bootstrap observations.');
    end;
  end;
  ...
  switch (treetype)                     % Get original dendrogram
    ...
    case 1,
      ...
      topo = [];
    otherwise
      error('  CLUSTER: invalid tree type.');
  end;
