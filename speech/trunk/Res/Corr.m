...
function [r,prob,CI_low,CI_high] = corr(X1,X2,kind,iter,CI_level)
  ...
  if (iter & (get_prob | get_CI))     % Randomized probs and CI intervals
    ...
    if (get_CI)                         % Randomize for confidence intervals
      ci = bootstrp('corrf',procs,iter,alpha,[X1 X2],[],[],P1,P2,1,0);
    end;
    if (get_prob)                       % Randomize for probabilities
      [c,pr] = bootstrp('corrf',procs,iter,alpha,[X1 X2],[],[],P1,P2,1,1);
    end;
