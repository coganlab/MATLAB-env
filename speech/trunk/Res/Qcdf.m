% QCDF: Cumulative distribution function for the Studentized Range Distribution Q
%       for r samples and v degrees of freedom, evaluated at q.  Modified from
%       Matlab function stdrcdf().
%
%     Usage: pr = qcdf(q,v,r,{right})
%
%       q =     test-statistic value.
%       v =     degrees of freedom.
%       r =     number of sample means.
%       right = optional boolean flag indicating, if true, that the right (upper)
%                 tail probability is to be estimated directly rather than by subtracting
%                 the left-tail probability from 1.  Likely to be more accurate if
%                 q is large and pr is close to 1.
%       ---------------------------------------------------------------------------------
%       pr =    probability of q.
%
...
function pr = qcdf(q, v, r, upper)
%STDRCDF Compute c.d.f. for Studentized Range statistic
%   F = QCDF(Q,V,R) is the cumulative distribution function for the
...
%   $Revision: 1.2 $  $Date: 2000/05/26 18:54:01 $
