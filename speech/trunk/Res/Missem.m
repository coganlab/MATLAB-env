...
%           maxtime =   optional number of minutes of elapsed time to allow 
...
%   3/14/04 - replaced finite() by isfinite().
%   4/8/04 -  changed maxtime specification from seconds to minutes.
...
function [X,M,C,propmiss,timeout,stop_time] = missem(Y,maxtime)
  ...
  if (~isempty(maxtime))              % Convert maxtime from minutes to seconds
    maxtime = 60*maxtime;
  end;
  ...
  for i = 1:p                         % For all variables,
    y = Y(isfinite(Y(:,i)),i);          % Eliminate missing values
    if (isempty(y))
      error('  MISSEM: all values missing for one or more variables');
    end;
...
%     C(i,i) = var(y);
  ...
  for i = 1:(p-1)                     % For all pairs of variables,
    for j = (i+1):p
      yi = isfinite(Y(:,i));              % Find pairwise available values
      yj = isfinite(Y(:,j));
      indx = find(yi & yj);
      y = Y(indx,[i j]);
...
%         c = y'*y / (length(indx)-1);    % Adjusted covariance
  ...
  while (do_again)
    ...
    for ip = 1:pattern                  % Est missing vals for obs for each pattern
      i = find(misspatn==ip);
      ni = length(i);
      varhave = find(isfinite(Y(i(1),:)));  % List of observed vars for this obs
      nhave = length(varhave);
      varmiss = find(~isfinite(Y(i(1),:))); % List of missing vars for this obs
