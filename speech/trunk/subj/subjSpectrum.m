...
%                           Defaults to []
%   CondParams.PropertyName() = 'Noisy' and/or 'NoResponse' and/or 'Incorrect'
...
%                           intervals
%   CondParams.PropertyName = 'String'  'Noisy','NoResponse', or Incorrect
...
%     CondParams.PropertyName{3} = 'Incorrect';
%     CondParams.PropertyValue{3} = 0;
...
%                                   
        ...
        if length(Ch)>1
            IEEG = IEEG(goodtrials,:,:); %-mean(IEEG(goodtrials,:,:),2);
            Spec = cell(1,length(Ch));
            if errorbar_flag
                Err = cell(1,length(Ch));
                for iCh = 1:length(Ch)
                    if length(goodtrials) > 1
                        ...
                        gIEEG = sq(IEEG(:,iCh,:)) - sq(mean(IEEG(:,AnalParams.RefChans{iCh},:),2));
...
%                                   
    ...
    case 'LinearRef'
        load([DUKEDIR '\' Subject.Name '\' Subject.Name '_w.mat']);
        %         channels = experiment.channels;
        %         [Channel_names{1:length(channels)}] = deal(channels.name);
        %         for iCh = 1:length(ReferenceChannelNums)
        %             try
        ...
        if length(Ch)>1
            ...
            if errorbar_flag
                ...
                for iCh = 1:length(Ch)
                    if length(goodtrials) > 1
                        gIEEG = sq(IEEG(:,iCh,:)) - sq(mean(IEEG(:,:,:),2));
                        gIEEG = sq(IEEG(:,iCh,:)) - sq(mean(w(iCh,:).*IEEG(:,:,:),2));
