function [Coh, Spec1, Spec2, Data, myTrials] = subjCoherency3D(Subject, CondParams, AnalParams)
% % Local Average Fix 130505 same reference (2 - corner issue fixed but less reference)
...
%                               Defaults to 0.05
%
% AnalParams.Shuffle = 0 default, for no, 1 for yes (NB TrialPooling will
% automatically go to 0 if 1 is selected
...
if ~isfield(AnalParams,'Shuffle') || isempty(AnalParams.Shuffle);
    AnalParams.Shuffle=0;
else
    AnalParams.TrialPooling=0;
end
if ~isfield(AnalParams,'nperm') || isempty(AnalParams.nperm) && AnalParams.Shuffle==1;
    AnalParams.nperm=100;
end
...
%     nextvals(64,3)=55;
        ...
        if size(Ch,1)>1
            ...
            for iChPair = 1:size(Ch,1)
              ...
              [refVAL1 refVAL2]=FindRefChan(Chs1,Chs2);
                 
                GAR_IEEG2A=sq(mean(GAR_IEEG(:,refVAL1,:),2));
                GAR_IEEG2B=sq(mean(GAR_IEEG(:,refVAL2,:),2));
              ...
              [refVAL1 refVAL2]=FindRefChan(Chs1,Chs2);
                ...
                GAR_IEEG2B=sq(mean(GAR_IEEG(:,refVAL2,:),2));
                gIEEG1 = sq(IEEG(:,Chs1,:)) - GAR_IEEG2A;
                for iPerm=1:AnalParams.nperm
                    tic
                gIEEG2 = sq(IEEG(shuffle(1:size(IEEG,1)),Chs2,:)) - GAR_IEEG2B;
                [CohA{iChPair}{iChPair2}, f, Spec1, Spec2] = tfcoh(gIEEG1,gIEEG2,tapers,sampling,dn,fk,pad,[],flag);
                Coh{iChPair}{iChPair2}(iPerm,:,:,:)=CohA;
                display('Right Yes Shuff') 
                toc
                end
                    end
                end
