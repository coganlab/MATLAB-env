function [Spec, Data, myTrials, Err] = subjSpectrum(Subject, CondParams, AnalParams)
%
    ...
    case 'Single-ended'
        if length(Ch) > 1
            ...
            if Errorbar_flag
                ...
                for iCh = 1:length(Ch)
                    [Spec{iCh},dum,Err{iCh}] = tfspec(sq(IEEG(:,iCh,:)),tapers,sampling,dn,fk,pad,Errorbar.P_val,flag,[],Errorbar);
                end
            else
                for iCh = 1:length(Ch)
                    Spec{iCh} = tfspec(sq(IEEG(:,iCh,:)),tapers,sampling,dn,fk,pad,[],flag);
                end
            end
        else
            IEEG = IEEG(goodtrials,:);
            if Errorbar_flag
                [Spec,dum,Err] = tfspec(IEEG,tapers,sampling,dn,fk,pad,Errorbar.P_val,flag,[],Errorbar);
            else
                Spec = cell(1,length(Ch)); % GBC 120924
                Spec = tfspec(IEEG,tapers,sampling,dn,fk,pad,[],flag);
            end
        end
        Data.IEEG = IEEG;
    case 'Grand average'
