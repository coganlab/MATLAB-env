function [Data, Trials] = subjIEEG(Subject, CondParams, AnalParams)
%
%   [Data, Trials] = subjIEEG(Subject, CondParams, AnalParams)
%
...
%   CondParams.bn      =   Alignment time in ms.
%                           Defaults to [-2e3,1e3]
%   CondParams.Conds    =   StartCode conds
%                           Defaults to []
...
switch Reference
    ...
    case 'Grand average'
        ...
        if length(Ch)>1
            IEEG = IEEG(goodtrials,:,:);
            gIEEG = zeros(size(IEEG),'single');
            for iCh = 1:length(Ch)
                gIEEG(:,iCh,:) = sq(IEEG(:,iCh,:)) - GAR_IEEG;
            end
        else
            IEEG = IEEG(goodtrials,:);
            gIEEG = IEEG - GAR_IEEG;
