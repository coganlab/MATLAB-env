function [Coh, Spec1, Spec2, Data, myTrials] = subjCoherency3B_Shuff(Subject, CondParams, AnalParams)
%
       ...
       % GAR_IEEG = trialIEEG(Trials, ReferenceChannelNums, Field, ...
        ...
        if size(Ch,1)>1
            ...
            for iChPair = 1:size(Ch,1)
                ...
                gIEEG2 = sq(IEEG(:,Chs2,:));
                if iPerm>1
                gIEEG2=gIEEG2(:,shuffle(1:size(gIEEG2,2)));
                end
                
                [Coh{iChPair}{iChPair2}{iPerm}, f, Spec1, Spec2] = tfcoh(gIEEG1,gIEEG2,tapers,sampling,dn,fk,pad,[],flag);
                display('Right')
