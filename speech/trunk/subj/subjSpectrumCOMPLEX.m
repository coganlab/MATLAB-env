function [Spec, Data, myTrials, Err] = subjSpectrumCOMPLEX(Subject, CondParams, AnalParams)
%
...
%                                   
                    ...
                    [Spec{iCh}, dum, Err{iCh}] = tfspecCOMPLEX(gIEEG,tapers,sampling,dn,fk,pad,Errorbar.P_val,flag,[],Errorbar);
                end
            else
                for iCh = 1:length(Ch)
                    if length(goodtrials) > 1
                        gIEEG = sq(IEEG(:,iCh,:));
                         Spec{iCh} = tfspecCOMPLEX(gIEEG,tapers,sampling,dn,fk,pad,[],flag);
            ...
            gIEEG = IEEG;
            if errorbar_flag
                [Spec,dum,Err] = tfspecCOMPLEX(gIEEG,tapers,sampling,dn,fk,pad,Errorbar.P_val,flag,[],Errorbar);
            else
                Spec = tfspecCOMPLEX(gIEEG,tapers,sampling,dn,fk,pad,[],flag);
            end
            Data.IEEG = gIEEG;
        end
    case 'Grand average'
