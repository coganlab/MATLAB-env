function [Coh, Spec1, Spec2, Data, myTrials] = subjCoherency3C(Subject, CondParams, AnalParams)
%  Local Average Fix 130505 variable reference (corners!)
...
%     nextvals(64,3)=55;
  ...
  newvals=zeros(64,8);
    for iChan=1:64;
        newvals(iChan,1)=iChan-9;
        newvals(iChan,2)=iChan-8;
        newvals(iChan,3)=iChan-7;
        newvals(iChan,4)=iChan-1;
        newvals(iChan,5)=iChan+1;
        newvals(iChan,6)=iChan+7;
        newvals(iChan,7)=iChan+8;
        newvals(iChan,8)=iChan+9;
    end
        ...
        if size(Ch,1)>1
            ...
            for iChPair = 1:size(Ch,1)
                ...
                Chs2=iChPair2;
                 ...
                 refch1=newvals(Chs1,:);
                 refch2=newvals(Chs2,:);
                 idx1=find(refch1>=1 & refch1<=64);
                 idx2=find(refch2>=1 & refch2<=64);
                 refch1B=refch1(idx1);
                 refch2B=refch2(idx2);
                 refch1B=setdiff(refch1B,test2);
                 refch2B=setdiff(refch2B,test1);
                 refch1C=setdiff(refch1B,refch2B);
                 refch2C=setdiff(refch2B,refch1B);
                 refch12C=intersect(refch1B,refch2B);
                 refch12C=setdiff(refch12C,cat(1,test1,test2));
                 refVAL1=cat(2,refch1C,refch12C(1));
                 refVAL2=cat(2,refch2C,refch12C(2));
