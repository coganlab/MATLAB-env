...
MARKERSIZE=[.5,4,8];
    ...
    set(fig,'Name','SpikeSort (Exp Def File)');
...
%     set(handles.PClabel3,'String','PC 3');
    ...
    set(handles.popMovieMarkersize,'String',{'Small','Medium','Large'});
    set(handles.popMovieMarkersize,'Value',3);
    set(handles.popSortMarkersize,'String',{'Small','Medium','Large'});
    set(handles.popSortMarkersize,'Value',3);
...
filename1 = [MONKEYDIR '/' day '/' rec '/rec' rec '.' mt '.sp.mat'];
filename2 = [MONKEYDIR '/' day '/' rec '/rec' rec '.' mt '.' num2str(mtch) '.sp.mat'];
if exist(filename2,'file')
    disp(['Loading from ' filename2]);
    load(filename2);
else
    disp(['Loading from ' filename1]);
    load(filename1);
    sp = sp{mtch};
end
...
NSKIP = 1;
MarkersizeValue = get(handles.popMovieMarkersize,'Value');
Markersize = MARKERSIZE(MarkersizeValue);

axes(handles.axDim12); cla;
 line(U_seg(1:NSKIP:end,Dim1),U_seg(1:NSKIP:end,Dim2),'LineStyle','none','Marker','.','Markersize',Markersize,'Color','b');
%plot(U_seg(1:NSKIP:end,Dim1),U_seg(1:NSKIP:end,Dim2),'.','Markersize',Markersize,'Color','b');

axes(handles.axDim13); cla;
 line(U_seg(1:NSKIP:end,Dim1),U_seg(1:NSKIP:end,Dim3),'LineStyle','none','Marker','.','Markersize',Markersize,'Color','b');
%plot(U_seg(1:NSKIP:end,Dim1),U_seg(1:NSKIP:end,Dim3),'.','Markersize',Markersize,'Color','b');

axes(handles.axDim23); cla;
 line(U_seg(1:NSKIP:end,Dim2),U_seg(1:NSKIP:end,Dim3),'LineStyle','none','Marker','.','Markersize',Markersize,'Color','b');
%plot(U_seg(1:NSKIP:end,Dim2),U_seg(1:NSKIP:end,Dim3),'.','Markersize',Markersize,'Color','b');
...
MarkersizeValue = get(handles.popSortMarkersize,'Value');
Markersize = MARKERSIZE(MarkersizeValue);
...
if (size(U_seg,2) >= Dim1) && (size(U_seg,2) >= Dim2) 
    for iClus = 1:14
        if ~SortData(FrameNumber).RemCell(iClus)
            ...
            if ~isempty(ind)
                 line(U_seg(ind(1:NSKIP:end),Dim1),U_seg(ind(1:NSKIP:end),Dim2),'LineStyle','none',...
                     'Marker','.','Markersize',Markersize,'Color',mycolors(iClus));
hold on
              %  plot(U_seg(ind(1:NSKIP:end),Dim1),U_seg(ind(1:NSKIP:end),Dim2),'.',...
              %      'Markersize',Markersize,'Color',mycolors(iClus));
            end
        end
    end
end
...
if (size(U_seg,2) >= Dim1) && (size(U_seg,2) >= Dim3) 
    for iClus = 1:14
        if ~SortData(FrameNumber).RemCell(iClus)
            ind = find(SortData(FrameNumber).Clu(:,2)==iClus);
            if ~isempty(ind)
                 line(U_seg(ind(1:NSKIP:end),Dim1),U_seg(ind(1:NSKIP:end),Dim3),'LineStyle','none',...
                     'Marker','.','Markersize',Markersize,'Color',mycolors(iClus));
hold on
               % plot(U_seg(ind(1:NSKIP:end),Dim1),U_seg(ind(1:NSKIP:end),Dim3),'.',...
               %     'Markersize',Markersize,'Color',mycolors(iClus));
            end
        end
    end
...
if (size(U_seg,2) >= Dim2) && (size(U_seg,2) >= Dim3)
    for iClus = 1:14
        if ~SortData(FrameNumber).RemCell(iClus)
            ind = find(SortData(FrameNumber).Clu(:,2)==iClus);
            if ~isempty(ind)
                 line(U_seg(ind(1:NSKIP:end),Dim2),U_seg(ind(1:NSKIP:end),Dim3),'LineStyle','none',...
                     'Marker','.','Markersize',Markersize,'Color',mycolors(iClus));
hold on
               % plot(U_seg(ind(1:NSKIP:end),Dim2),U_seg(ind(1:NSKIP:end),Dim3),'.',...
               %     'Markersize',Markersize,'Color',mycolors(iClus));
            end
        end
    end
...
% --------------------------------------------------------------------
function varargout = popMovieMarkersize_CreateFcn(h, eventdata, handles, varargin)

function varargout = popSortMarkersize_CreateFcn(h, eventdata, handles, varargin)
...
Trials = get(handles.frTrials,'UserData');
[day,rec,mt,ch] = RecInformation(handles);
if(length(Trials)>0) % if Labview behavior
    % in order to get threshold, use trial number which would approx
    % correspond to the center of this frame (pretty arbitrary)
    k = floor((FrameNumber/length(MovieData))*length(Trials) + 1/(length(MovieData)*2));
    trial = Trials(k);    
    load([MONKEYDIR '/' day '/' rec '/rec' rec '.Rec.mat'])
    Raw = RawTrial(trial,mt,ch,'End',[0,1e3]);
