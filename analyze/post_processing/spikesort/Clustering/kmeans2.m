function [centres, options, post, errlog] = kmeans2(data, k, centres, options)
%KMEANS	Trains a k means cluster model.
%
%	Description
%	 CENTRES = KMEANS(DATA, K, CENTRES, OPTIONS) uses the batch K-means
...
%
%	[CENTRES, OPTIONS, POST, ERRLOG] = KMEANS(DATA, OPTIONS, CENTRES)
...
%
%	OPTIONS(14) is the maximum number of iterations; default 30.
%
%	See also
%	GMMINIT, GMMEM
%
...
for n = 1:niters
  ...
  for j = 1:ncentres
    if (num_points(j) > 0)
      centres(j,:) = sum(data(post(:,j)>0,:), 1)/num_points(j);
    end
  end
