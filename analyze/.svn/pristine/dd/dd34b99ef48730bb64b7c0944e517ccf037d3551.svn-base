...
%        return
        ...
        Raw(iTask).Target =  getMovementChoice(Raw(iTask).Trials);
...
for iDir = 1:8 
  num_choice_trials = getMovementChoice(Trials);
  numTrials(iDir) = length(find(num_choice_trials == iDir));
end
...
%bn
PDCoh = sessChoiceCoherency(Sess,Task,Field,bn,{[PD]},[],[],[.5,2]);
aPDCoh = sessChoiceCoherency(Sess,Task,Field,bn,{[aPD]},[],[],[.5,2]);
