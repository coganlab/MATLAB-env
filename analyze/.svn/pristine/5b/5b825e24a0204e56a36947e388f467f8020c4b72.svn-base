function [rho,Xspace,Yspace,time,f_axis] = fccaxy_tr(X, Y, tapers, sampling, dn, fk)
%FCCAXY_TR  Space-frequency canonical correlation using multitaper methods with
%      trial averaging over the first index.
%
% [RHO, XSPACE, YSPACE, F] = FCCAXY_TR(X, Y, TAPERS, SAMPLING, DN, FK) 
%
...
if ~t                       % process the single window case 
    ...
    rho = zeros(nwin,nf,min(K*nTr,nChX));
