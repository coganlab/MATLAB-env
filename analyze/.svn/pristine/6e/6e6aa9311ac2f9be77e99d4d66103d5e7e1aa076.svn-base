function Results = sessTestCLDAKalmanJoint(Session, Tower, JointList, kalmanparams, bin_time, allchannels,joint_file_name,Task_Targets)
%
...
%           kalmanparams.lag = 10;
%           kalmanparams.C = 4
%           kalmanparams.half_life = 90 (secs)
%           kalmanparams.batch_time = 120 (secs)
%           bin_time  =  Test and train bin width (in minutes)
%           joint_file_name = file name of jnt file
%           Task_Targets = file aligned to the neural data of all the
%           targets.
%  Outputs:  Results =
%
...
addpath('/mnt/raid/MuJoCo/MuJoCo_05/matlab/mex')
day = sessDay(Session);
...
if nargin < 4 || isempty(kalmanparams)
    ...
    kalmanparams.half_life = 90;
    kalmanparams.batch_time = 120;
end
...
if ~isfield(kalmanparams, 'half_life')
    kalmanparams.half_life = 90;
end
if ~isfield(kalmanparams, 'batch_time')
    kalmanparams.batch_time = 120;
end
half_life = kalmanparams.half_life;
batch_time = kalmanparams.batch_time;
...
load([path, '.TargetInformation.mat'])
...
%hack to ensure same timing - should fix
[dum1, target_info, dum2] = bin(pk,  binwidth, target_information);
...
for iLoop = 1:loops
    ...
    [jointPred jointVar] = CLDAkalmanFilter(tmp,A,C,Q,R,zeros(length(A),1),Q, kalmanparams,target_info, jointTest);
    jointTest = jointTest';
