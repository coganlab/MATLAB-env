        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn && Ind_StartOn~=size(evs,1)
                ...
                try
                    ...
                    Events.RewardDur(trial,:) = lv_events(lv_trial-1,2:5);
                    Events.Delta(trial) = st - lv_events(lv_trial-1,1);
                    Events.RewardDist(trial,:) = lv_events(lv_trial-1,6:9);
                end
                ...
                try
                    ...
                    Events.EyeTargetLocation(trial,:) = lv_target(lv_trial-1,4:5);
                    Events.HandTargetLocation(trial,:) = lv_target(lv_trial-1,2:3);
                    Events.TargetDelta(trial) = st - lv_target(lv_trial-1,1);
...
%                     end
                ...
                try
                    ...
                    Events.MinReachRT(trial,:) = lv_acquire(lv_trial-1,2);
                    Events.MaxReachRT(trial,:) = lv_acquire(lv_trial-1,3);
                    Events.MinReachAq(trial,:) = lv_acquire(lv_trial-1,4);
                    Events.MaxReachAq(trial,:) = lv_acquire(lv_trial-1,5);
                    Events.MinSaccadeRT(trial,:) = lv_acquire(lv_trial-1,6);
                    Events.MaxSaccadeRT(trial,:) = lv_acquire(lv_trial-1,7);
                    Events.MinSaccadeAq(trial,:) = lv_acquire(lv_trial-1,8);
                    Events.MaxSaccadeAq(trial,:) = lv_acquire(lv_trial-1,9);
                    Events.MinSaccade2RT(trial,:) = lv_acquire(lv_trial-1,10);
                    Events.MaxSaccade2RT(trial,:) = lv_acquire(lv_trial-1,11);
                    Events.MinSaccade2Aq(trial,:) = lv_acquire(lv_trial-1,12);
                    Events.MaxSaccade2Aq(trial,:) = lv_acquire(lv_trial-1,13);
                end
...
%                     [x lv_trial] = min(tOffset);
                    % [ x tOffset(lv_trial-1) ]
                    
                    Events.T1T2Locations(trial,:,:) = [lv_target(lv_trial-1,2:3); lv_target(lv_trial-1,4:5) ];
                    
                    % if lv_trial>41 % 500
                    % if Events.Success(trial) & Events.RewardTask(trial)==9
%                         

% if Events.RewardTask(trial)==9
%                     figure(1); hold on;
%                     plot(lv_target(lv_trial-1,2),lv_target(lv_trial-1,3),'ro');
%                     plot(lv_target(lv_trial-1,4),lv_target(lv_trial-1,5),'g+');
%                     
...
%                     end
                    ...
                    Events.T1T2Delta(trial) = st - lv_target(lv_trial-1,1);
                end
                ...
                try
                    ...
                    Events.BrightVals(trial,:) = [lv_bright(lv_trial-1,2) lv_bright(lv_trial-1,3)]/100;
                    Events.BrightDist(trial,:) = [lv_bright(lv_trial-1,4) lv_bright(lv_trial-1,5) lv_bright(lv_trial-1,6) lv_bright(lv_trial-1,7)]/100;
                end
