    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                ...
                Events.RewardBlock(trial) = evs(Ind_StartOn-7,2) - 30+1;  %  Which mag or prob block encoded - need printout
                Events.RewardTask(trial) = evs(Ind_StartOn-8,2) - 10+1;   %  Which type of reward task - simple, 2t, 4t etc
                ...
                if find(evs(:,2)==4)
                    ...
                    if length(display_evs) & (Events.LEDBoard(trial) == 2) & (Events.TaskCode(trial) == 11) %detects the LED change during DS adaptation
                        Events.TargMove(trial) = min(display_evs(find(display_evs(:,2)>evs(index(1),3)),2));
                    end
                end
