function [coh_ptx, pval_ptx, tfsp_X, tfsp_dN, rate, f] = ...
   ptfcohpval_ptx(X, dN, tapers, sampling, dn, fk, pad)
%PTFCOHPVAL_PTX  Moving window time-frequency hybrid process coherency using
%parfor
%
%  [COH_PTX, PVAL_PTX, TFSP_X, TFSP_DN, RATE, F] = ...
%		PTFCOHPVAL_PTX(X, dN, TAPERS, SAMPLING, DN, FK, PAD, PVAL, FLAG) 
%
...
parfor win = 1:nwin
    ...
    %         SX = zeros(nch,diff(nfk),K,'single');
    %         SdN = zeros(nch,diff(nfk),K,'single');
    %         SXdN = zeros(nch,diff(nfk),K,'single');
