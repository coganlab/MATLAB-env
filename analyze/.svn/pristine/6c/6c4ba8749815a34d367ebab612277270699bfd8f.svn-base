    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                ...
                Events.EyeTarget(trial) = evs(Ind_StartOn-5,2)-10+1; %Added for v3.15
                Events.RewardConfig(trial) = evs(Ind_StartOn-6,2)-30+1;  %   Which configuration of alternatives
                Events.Choice(trial) = evs(Ind_StartOn-7,2) - 10+1;       %  Free or instructed choice
                Events.RewardBlock(trial) = evs(Ind_StartOn-8,2) - 30+1;  %  Which mag or prob block encoded - need printout
                Events.RewardTask(trial) = evs(Ind_StartOn-9,2) - 10+1;   %  Which type of reward task - simple, 2t, 4t etc
                Events.Target(trial) = evs(Ind_StartOn-10,2)-30+1;
                Events.StartHand(trial) = evs(Ind_StartOn-11,2)-10+1;
                Events.StartEye(trial) = evs(Ind_StartOn-12,2)-30+1;
                Events.HandCode(trial) = evs(Ind_StartOn-13,2)-10;
                Events.Joystick(trial) = evs(Ind_StartOn-14,2)-50;
                Events.TaskCode(trial) = evs(Ind_StartOn-15,2)-10;
