function Session = StoSFF(InputSession, Chamber1, Ch1, Contact1, Chamber2, Ch2, Contact2, Task, N)
%
        ...
        if ~isempty(Ch2)
            if ~isempty(Chamber2)
                if ~isempty(Contact2)
                    if ~isempty(Ch1)
                        if ~isempty(Contact1)
                            if SpikeSessNum == InputSession{6}(1) && (strcmp(FieldChamber1,Chamber1) || ...
                                    (strcmp(FieldChamber1(1:2),Chamber1(1:2)) && ...
                                    strcmp(FieldChamber1(end),Chamber1(end)))) && FieldCh1==Ch1 && FieldContact1==Contact1 && ...
...
if ~isempty(Session)
    disp([num2str(length(Session)) ' SpikeFieldField Sessions']);
    %%%  Does the Task selection
    if ~isempty(Task)
        if ischar(Task); Task = {Task}; end
        load([MONKEYDIR '/mat/SpikeFieldField/SpikeFieldField_NumTrials.mat']);
        for iSess = 1:length(Session)
        ...
        disp([num2str(length(Session)) TaskString ' SpikeFieldField sessions']);
    else
        disp([num2str(length(Session)) ' SpikeFieldField sessions']);
    end
else
