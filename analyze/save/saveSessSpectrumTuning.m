function saveSessSpectrumTuning(SpectrumTuningData,CondParams,AnalParams,forceSave);
...
Session = SpectrumTuningData.Session;

doCoherence = 0;
sessType = getSessionType(Session);
switch sessType
    case {'Field','Multiunit','Spike'}
      dirPath = [ MONKEYDIR '/mat/' sessType '/tuning' ];
end
...
if p==0 | forceSave
  ...
  eval(['save ' dirPath '/' fNameRoot '.d' num2str(pMax+1) '.mat SpectrumTuningData']);
