function [FigureHandle, ROC, se] = plotSelectionROC(Onset,ModelType)
...
for iType = 1:length(Onset.Results)
    ...
    t = [1:Dt:MaxTime];
    hold on;
    h = patch([t,t(end:-Dt:1)],[ROC+2*se,ROC(end:-1:1)-2*se(end:-1:1)],0.7*[1,1,1]);
