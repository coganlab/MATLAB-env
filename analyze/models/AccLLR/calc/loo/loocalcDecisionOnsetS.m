function [Results, Data , Model] = loocalcDecisionOnsetS(Session, InputParams, decimate)
%
%  [Results, Data, Model] = loocalcDecisionOnsetS(Session, InputParams,decimate);
...
%
%           decimate - 0 -for no decimation any number greater than 1 will
%           cause firing rates to be decimated by that number
%
...
if iscell(Session{1})
    ...
    for iSess = 1:nSess
        ...
        SpikesNull{iSess} = trialSpike(NullTrials{iSess}, SysS, ChS,1, ClS, InputParams.Null.Field, InputParams.Null.bn);
        SpikesEvent{iSess} = trialSpike(EventTrials{iSess}, SysS, ChS,1, ClS, InputParams.Event.Field, InputParams.Event.bn);     
        if(decimate > 1)
            tmpSpike = SpikesEvent{iSess};
                tmp1 = 0;
                tmp2 = 0;
            for iTrial = 1:length(tmpSpike)
                %disp(['event session: ' num2str(iSess) ' trial: ' num2str(iTrial)])
                tmpTrialSpike = tmpSpike{iTrial};
                numSpikes = length(tmpTrialSpike);
                tmp1 = tmp1+numSpikes;
                for iSpike = 1:numSpikes
                    if(rand > 1/decimate)
                        tmpTrialSpike(numSpikes+1-iSpike) = [];
                    end
                end
                tmp2 = tmp2 + length(tmpTrialSpike);
                tmpSpike{iTrial} = tmpTrialSpike;
            end
            SpikesEvent{iSess} = tmpSpike;
            
            tmpSpike = SpikesNull{iSess};
            for iTrial = 1:length(tmpSpike)
                tmpTrialSpike = tmpSpike{iTrial};
                numSpikes = length(tmpTrialSpike);
                for iSpike = 1:length(tmpTrialSpike)
                    if(rand > 1/decimate)
                        tmpTrialSpike(numSpikes+1-iSpike) = [];
                    end
                end
    ...
    SpikesNull = trialSpike(NullTrials, SysS, ChS,1, ClS, InputParams.Null.Field, InputParams.Null.bn);
    SpikesEvent = trialSpike(EventTrials, SysS, ChS,1, ClS, InputParams.Event.Field, InputParams.Event.bn);
    if(decimate > 1)
        tmpSpike = SpikesEvent;
        tmp1 = 0;
        tmp2 = 0;
        for iTrial = 1:length(tmpSpike)
            %disp(['event session: ' num2str(iSess) ' trial: ' num2str(iTrial)])
