...
for iTask = 1:length(InputParams.Event.Task)
    ...
    tmpParams = InputParams;
    tmpParams.Task = InputParams.Event.Task{iTask};
    Trials = Params2Trials(SessionTrials, tmpParams);
    if ~isempty(Trials)
...
for iTask = 1:length(InputParams.Event.Task)
    ...
    tmpParams.Task = InputParams.Null.Task{iTask};
    Trials = Params2Trials(SessionTrials, tmpParams);
    if ~isempty(Trials)
        Trials = Trials([Trials.Choice]==2);
        TotalTrials = [TotalTrials Trials];
