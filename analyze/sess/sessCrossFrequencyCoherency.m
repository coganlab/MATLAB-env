function [Coh,S,f,t,Data] = sessCrossFrequencyCoherency(Sess,CondParams, AnalParams)
%
%   [Coh,S,f,t,Data] = sessCrossFrequencyCoherency(Sess,CondParams, AnalParams)
%
...
if length(Trials{1}) > 3
    for iTaskComp = 1:length(Trials)
        ...
        disp(['Calculating ' Type ' cross-frequency coherency with N = ' ...
		...
		Contact = sessContact(Sess);
                ...
                tic; [Coh{iTaskComp}, f, S{iTaskComp}] = ...
                    cf_coherency(Lfp, [N,W], sampling_rate, fk, pad); toc
                Data.Lfp = Lfp;
        end
    end
else
    ...
    Coh = {zeros(diff(nfk),diff(nfk),'single')};
    f=0;
end
