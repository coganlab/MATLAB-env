function [p,D,PD] = sessTestCohNull(Sess,CondParams,AnalParams)
...
switch Type
    case 'FieldField'
        ...
        X = trialLfp(Trials, Sys{1}, Ch(1), Contact(1), CondParams.Field, ...
            [CondParams1.bn-N./2*1e3,CondParams.bn+N./2*1e3],MonkeyDir);
        Y = trialLfp(Trials, Sys{2}, Ch(2), Contact(2), CondParams.Field, ...
            [CondParams1.bn-N./2*1e3,CondParams.bn+N./2*1e3],MonkeyDir);
        Xk = zeros(ntr*K, diff(nfk));
        Yk = zeros(ntr*K, diff(nfk));
        meanX = sum(X)./ntr; meanY = sum(Y)./ntr;
        for tr=1:ntr
            tmp = (X(tr,:) - meanX).';
            xk = fft(tapers(:,1:K).*tmp(:,ones(1,K)),nf)';
            Xk((tr-1)*K+1:tr*K,:) = xk(:,nfk(1)+1:nfk(2));
            tmp = (Y(tr,:) - meanY).';
            yk = fft(tapers(:,1:K).*tmp(:,ones(1,K)),nf)';
            Yk((tr-1)*K+1:tr*K,:) = yk(:,nfk(1)+1:nfk(2));
        end
        S_X = sum(abs(Xk).^2); S_Y = sum(abs(Yk).^2);
        coh = sum(Xk.*conj(Yk))./sqrt(S_X.*S_Y);  %def'n of coherence
        
        D = sum(abs(coh))./(nfk(2)-nfk(1));
        GX = [Xk]; GY = [Yk]; PD = zeros(1,5e3);
        for iPerm = 1:nPerm
            NP = randperm(size(GX,1));
            %PS_X1 = sum(abs(GX(N1,:)).^2); PS_Y1 = sum(abs(GY(N1,:)).^2);
            Pcoh = sum(GX(NP,:).*conj(GY(:,:)))./sqrt(S_X.*S_Y);
    ...
    case {'SpikeField','MultiunitField'}
        ...
        dN = trialSpike(Trials, Sys{1}, Ch(1), Contact(1), Cl, CondParams.Field, ...
