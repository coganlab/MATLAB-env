function sessPrintNormalizedSpectrogram(Sess,CondParams, AnalParams)
%,Task,Field,bn,time,freq, tapers
%   sessPrintNormalizedSpectrogram(Sess,CondParams, AnalParams)
%
...
%   CondParams.Choice  = Defaults to 0;
%
...
baseline = 1:10;
base = sq(mean(sq(mean(spectun(:,baseline,:),1)),1));
dims = size(spectun);
for i = 1:8
    spectun(i,:,:) = sq(spectun(i,:,:))./base(ones(1,dims(2)),:); 
end
...
plot_title = [day ', Sess: ' num2str(Sess{6}) ', ' area  ', Depth: ' num2str(Sess{5}{1}(1)) ', ' Task{1} ' Rec: ' ...
    recordings{1} ', Bin: ' num2str(time) ' PD:' num2str(PD)];
