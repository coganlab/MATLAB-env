function [Fig]=sessPrintSFCoh(Sess,Task,Field,bn,time,freq)
%
%   Fig = sessPrintSFCoh(Sess,Task,Field,bn,time,freq)
%
...
global MONKEYNAME MONKEYDIR CONTROLTASKLIST experiment
...
%PossibleTasks={'DelReachFix','DelSaccadeTouch','DelReachSaccade','DelReach','DelSaccade','DelSaccadethenReach','TOA','MemoryReachSaccade','MemorySaccade','MemorySaccadeTouch'};
...
for iDir=1:8 
    [Coh(iDir,:,:),S1,S2,f,t,numTrials(iDir)] = sessCoherency(Sess,Task,Field,bn,{[iDir]});
end
...
minlevel=min(min(min(abs(Coh(PD,:,:)))),min(min(abs(Coh(aPD,:,:)))));
maxlevel=max(max(max(abs(Coh(PD,:,:)))),max(max(abs(Coh(aPD,:,:)))));

if max(abs(Coh)) > 0
    if PD>4
        subplot(3,1,2)
        tvimage(abs(Coh(aPD,:,1:150)),'CLim',[minlevel,maxlevel])
        title(['SF Coh ' num2str(numTrials(aPD)) ' trials  Dir = ' num2str(aPD)])
        colorbar
        subplot(3,1,3)
        tvimage(abs(Coh(PD,:,1:150)),'CLim',[minlevel,maxlevel])
        title(['SF Coh ' num2str(numTrials(PD)) ' trials  Dir = ' num2str(PD)])
        colorbar
    else
        subplot(3,1,2)
        tvimage(abs(Coh(PD,:,1:150)),'CLim',[minlevel,maxlevel])
