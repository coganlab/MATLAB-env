function Session = StoSF(InputSession, Chamber, Ch, Contact, Task, N)
%
%   Session = StoSF(InputSession,Chamber,Ch,Contact,Task,N)
%
        ...
        if ~isempty(Ch)
            if ~isempty(Contact)
                if SpikeSessNum == InputSession{6}(1) ...
                        ...
                        strcmp(FieldChamber(end),Chamber(end)))) && FieldCh==Ch && FieldContact==Contact
                    Ind = [Ind,iSess];
                end
            else
                if SpikeSessNum == InputSession{6}(1) ...
                        && (strcmp(FieldChamber,Chamber) || ...
                        (strcmp(FieldChamber(1:2),Chamber(1:2)) && ...
                        strcmp(FieldChamber(end),Chamber(end)))) && FieldCh==Ch
                    Ind = [Ind,iSess];
                end
            end
        else
