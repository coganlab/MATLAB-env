...
if nargin < 2 tapers = dpsschk([nt, 5, 9]); end 
  	...
  	rate(ch) = mean(sum((ntapers(:,1:K).*tmp(:,ones(1,K))).^2,1));
        ...
        if errorchk	%  Estimate error bars using Jacknife
          for ik = 1:K
            ...
            dNj = dNk(indices,:);
            jlsp(ik,:) = log(mean(dNj.*conj(dNj),1));
          end
          lsig = sqrt(K-1).*std(jlsp,1);
%          p = 		%   Determine the scaling factor
%          err(1,ch,:) = exp(log(spec(ch,:))+p.*lsig);
%          err(2,ch,:) = exp(log(spec(ch,:))-p.*lsig);
         end
    end
end
...
%	 p = 		%   Determine the scaling factor
%      	 err(1,:) = exp(log(spec)+p.*lsig);
%        err(2,:) = exp(log(spec)-p.*lsig);
   end
end
