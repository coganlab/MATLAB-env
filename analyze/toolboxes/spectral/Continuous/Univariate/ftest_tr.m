function [fs, cs, f, var] = ftest_tr(X, tapers, sampling, fk, pad, pval, flag)
% FTEST_TR computes the F-statistic for sine wave in locally-white noise.
%
% [FS, CS, F, ERR] = FTEST_TR(X, TAPERS, SAMPLING, FK, PAD, PVAL, FLAG)
%
...
%
%	   FLAG = 0:	calculate FTEST seperately for each channel.
%	   FLAG = 1:	calculate FTEST by pooling across channels. 
%		Defaults to FLAG = 0;
%
%  Outputs: FS		=  F-statistic for X in [Space, Freq] form.
%  	    CS		=  Line amplitude for X in [Space, Freq] form. 
%	    F		=  Units of Frequency axis for FS and CS.
%	    ERR 	=  Error bars for CS in [Hi/Lo, Space, Freq]
%			   form given by a Jacknife-t interval for PVAL.
%

% Modification History:  
%           Written by:  Bijan Pesaran, 2001
...
%  for ch = 1:nch
     ...
     %xk = xk(:,nfk(1):nfk(2));
     xtr_k = sp_proj(X,tapers,1e3,fk,pad);
     Sk = abs(xk).^2;
     sp = sum(Sk);
