function [spec, f, err] = ptfspec(X, tapers, sampling, dn, fk, pad, pval, flag, contflag, Errorbar)
%PTFSPEC  Moving window time-frequency spectrum using multitaper techniques.
%
...
if ~flag				% No pooling across trials
    ...
    if ~errorchk				%  Don't estimate error bars
        ...
        for win = 1:nwin
            ...
            if contflag
                tmp = detrend(X(:,dn*(win-1)+1:dn*(win-1)+N))';
            else
