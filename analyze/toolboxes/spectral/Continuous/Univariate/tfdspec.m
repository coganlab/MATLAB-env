function [tfdsp_df,tfdsp_dt,tfsp]=tfdspec(X, tapers, dn, nf, fk, ...
					sampling, flag)
%TFDSPEC Time and Frequency spectral derivatives
%
%  [TFDSP_DF,TFDSP_DT,TFSP]=TFDSPEC(X) computes the time and
%  frequency derivative spectral estimates.  
%
%  [TFDSP_DF, TFDSP_DT, TFSP, F] = TFDSPEC(X, TAPERS, DN, NF, FK, ...
%                                        SAMPLING, FLAG) 
%
%	Defaults:
...
if nargin > 5 tapers(1) = tapers(1).*sampling; end
N = tapers(1);
if nargin < 4; nf = 2.^(nextpow2(N+1)+1); end
