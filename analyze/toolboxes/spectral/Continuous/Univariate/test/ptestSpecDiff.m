function [p,D,PD,S_X1,S_X2] = ptestSpecDiff(X1,X2,tapers,sampling,fk,nPerms)
%;Task,Field,bn,conds,tapers,dn,delay)
%
%   [p,D,PD,S_X1,S_X2] = ptestSpecDiff(X1,X2,tapers,sampling,fk)
%
%   X1 
%   X2
...
if ntr1 < 2 || ntr2 < 2
    ...
    error(['Not enough trials ' num2str(ntr1) ':' num2str(ntr2)])
        ...
        parfor tr = 1:ntr1
            tmp1 = X1(tr,:)';
            xk = fft(tapers.*tmp1(:,ones(1,K)),nf)';
            Xk1(tr,:,:) = xk(:,nfk1+1:nfk2);
        end
        parfor tr = 1:ntr2
            tmp2 = X2(tr,:)';
            xk = fft(tapers.*tmp2(:,ones(1,K)),nf)';
            Xk2(tr,:,:) = xk(:,nfk1+1:nfk2);
        end
