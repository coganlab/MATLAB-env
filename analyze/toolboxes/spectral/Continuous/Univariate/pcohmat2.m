function [C_mat, f, S_mat] = ...
    pcohmat2(X, tapers, sampling, fk, pad, flag) 
...
%				Defaults to 2.
%
%	   FLAG = 0:	calculate C_MAT separately for each channel/trial.
...
if length(sX) == 2	%  Single trial
    ...
    for ch1 = 2:nch  %  Looping across channels
        disp(['Channel 1 is ' num2str(ch1)]);
        for ch2 = 1:ch1-1
            disp(['Channel 2 is ' num2str(ch2)]);
            id = id+1;
            nodes(id,:) = [ch1,ch2];
            cmd = ['[cohtemp, dum, s_xtemp] = coherency(X(' num2str(ch1) ...
                    ',:),X(' num2str(ch2) ',:), tapers, ' num2str(sampling) ...
