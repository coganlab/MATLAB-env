...
if waveflag == 0; 
    tic,fprintf('TFWAVELET: Data Transform\n'); 
elseif waveflag ==1;
    tic,fprintf('TFWAVELET: Checking Parameters\n');
end
...
foi =  FREQPAR.foi;
bw = FREQPAR.bw;
foi_min = 2*foi/(2^bw+1); % Freq width low end
foi_max = 2*foi/(2^-bw+1); % Freq width high end
dfoi = foi_max-foi_min; % 2*std in foi domain % bandwidth for each frequency
timewin = 6/pi./dfoi; % width in time of windows per frequency, note 6 is fixed here.
timewin = round(timewin*1000)/1000; % ??
toffset = timewin(1)/2; % sets first time point
...
if dospec
    ...
    if isempty(FREQPAR.win_centers)
        if ~isfield(FREQPAR,'stepsize') ;
        tshift = timewin(end)/2;
        WAVEPAR.stepsize=tshift;
        else
            tshift=FREQPAR.stepsize;
        end
