function [C_list, f] = ...
    tfcohlist(X, list, tapers, sampling, dn, fk, pad, flag) 
% TFCOHLIST calculates the coherency for a list of time series.
%
% [C_LIST, F] = ...
%	TFCOHLIST(X, LIST, TAPERS, SAMPLING, DN, FK, PAD, FLAG)
%
...
%  Written by:  Bijan Pesaran Caltech 2002
%
...
dn = dn.*sampling;
nfk = floor(fk./sampling.*nf);
nwin = floor((nt*sampling-N)./dn);           % calculate the number of windows

nlist = size(list,1);

if length(sX) == 2	%  Single trial
  C_list = zeros(nlist,nwin,diff(nfk));
  for n = 1:nlist
    ch1 = list(n,1);
    ch2 = list(n,2);
    [coh] = tfcoh(X(ch1,:), X(ch2,:), tapers, sampling, dn./sampling, fk, pad, flag);
    C_list(n,:,:) = sq(coh);
  end
end
