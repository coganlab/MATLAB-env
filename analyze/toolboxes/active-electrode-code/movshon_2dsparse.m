...
load('\\tneuro-think\Users\TNeuro\Desktop\Animal Experiment - 2012-12-04\done\test_014.mat')
...
level_set_delay = 0.05;    % how many seconds offset from the trigger should we look for the voltage level corresponding to this orientation or location

start_sec = .1; % how many seconds into the file to start
stop_sec = .2; % how many seconds to discard from the end of the file
% numRow - number of electrode rows
% numCol - number of electrode columns
% numChan - number of analog channels
% Fs - sampling rate of the data
data_type = '2DSPARSE'; % string describing the experiment

SecsPre = 0.1;  % how many seconds prior to the trigger to average
SecsPost = 0.2; % how many seconds after the trigger to average

x_assignments = x;
y_assignments = y;
c_assignments = c;
c_type = 1; % which color 

[avg_eps,count,x] = vep_average_movshon(data, start_sec, stop_sec, SecsPre, SecsPost, numRow, Fs, data_type, x_assignments, number_x_levels, invert_x, y_assignments, number_y_levels, invert_y, c_assignments, c_type, ttl_chan, trigger_edge, ttl_threshold, level_set_delay);
