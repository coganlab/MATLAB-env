%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% JV - converted into a function to return data
%   Reads in TDMS files and outputs one analog channel of data
%   inputs:
%       filename - full path and filename of tdms file to work on
%       whichChan - which channel of analog data to extract at this time
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [chanvals]=ReadFile(filename)
...
% JV - add local paths relative to the SVN code project for ease of use
if strcmp(computer, 'PCWIN64')
NI_TDM_DLL_Path = strcat(pwd,'\MATLAB TDM Example\dev\bin\64-bit\nilibddc.dll');
NI_TDM_H_Path = strcat(pwd,'\MATLAB TDM Example\dev\include\64-bit\nilibddc_m.h');
end
if strcmp(computer, 'PCWIN')
NI_TDM_DLL_Path = strcat(pwd,'\MATLAB TDM Example\dev\bin\32-bit\nilibddc.dll');
NI_TDM_H_Path = strcat(pwd,'\MATLAB TDM Example\dev\include\32-bit\nilibddc_m.h');
end
libname = 'nilibddc';
...
% JV - modified to take in full filename from the function call
Data_Path=fullfile(filename);
...
if strcmp(computer, 'PCWIN')
    ...
    % JV - no looping over the channels, just grab one
