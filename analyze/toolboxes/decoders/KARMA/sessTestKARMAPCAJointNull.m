function [Results, PCAParams, myJoint] = sessTestKARMAPCAJointNull(Sess, Tower, JointList, karmaparams, bin_time, PCAParams, allchannels)
%
%  [Results, PCAParams] = sessTestKARMAJointNull(Sess, Tower, JointList, karmaparams, bin_time, PCAParams, allchannels)
%
...
if(length(recs) == 1)
    ...
    for iJoint = 1:size(PCAJoint,2)
        randomized_ind = randperm(size(PCAJoint,1));
        PCAJoint(:,iJoint) = PCAJoint(randomized_ind,iJoint);
    end
    if iscell(Tower)
        pks = {};
    ...
    for iRec = 1:length(recs)
        ...
        % Shuffle joint positions
        for iJoint = 1:size(Joint,2)
            randomized_ind = randperm(size(Joint,1));
            Joint(:,iJoint) = Joint(randomized_ind,iJoint);
        end
