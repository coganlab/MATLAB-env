function Results = sessTestKARMAJoint(Session, Tower, JointList, karmaparams, bin_time, allchannels,joint_file_name)
%
%  Results = sessTestKARMAJoint(Session, Tower, JointList, karmaparams, bin_time, allchannels,joint_file_name)
%
...
if nargin < 6 % rabadi edit 12/05/2014 (default did not previously exist)
    allchannels = 0;
end
...
if(length(recs) == 1)
    ...
    for iRec = 1:length(recs)
        ...
        jointdata_tmp = cell(1, nJoint);
        for iJoint = 1:nJoint
          jointdata_tmp{iJoint} = Joint{iJoint}';
        end
...
for iLoop = 1:loops
    ...
    [LinearJoint, LinearParams] = linearizeJointAngles(jointTrain);
    model = fitKARMA(spikeTrain, LinearJoint, neural_lag, observed_lag, gamma, C); %10,10 worked better
    %jointPred = predictKARMA(spikeTest,zeros(nJoint,1),model);
    LinearJointPred = fast_karma_predict(spikeTest,model);
    jointPred = unlinearizeJointAngles(LinearJointPred, LinearParams);
    jointPred = jointPred';
    ...
    [LinearJoint, LinearParams] = linearizeJointAngles(jointTrain);
    model = fitKARMA(spikeTest, LinearJoint,  neural_lag, observed_lag, gamma, C); %10,10 worked better
    %jointPred = predictKARMA(spikeTrain,zeros(nJoint,1),model);
    LinearJointPred = fast_karma_predict(spikeTrain,model);
    jointPred = unlinearizeJointAngles(LinearJointPred, LinearParams);
    jointPred = jointPred';
