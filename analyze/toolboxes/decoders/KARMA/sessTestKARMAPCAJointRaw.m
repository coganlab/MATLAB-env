function [Results, PCAParams] = sessTestKARMAPCAJointLFP(Sess, Tower, JointList, karmaparams, bin_time, PCAParams, allchannels,joint_file_name)
%
...
whos Lfp
freq = [1:5,7:14,16:19,21:29,80];
Lfp(:,:,freq) =[];
whos Lfp
%[PCALfp, PCAParamsLFP] = processPCALFP(lfpspec,PCAParamsLFP);
Lfp = shiftdim(Lfp,1);
PCALfp = reshape(Lfp,size(Lfp,1),size(Lfp,2).*size(Lfp,3));
