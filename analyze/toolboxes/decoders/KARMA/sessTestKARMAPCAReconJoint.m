function Results = sessTestKARMAPCAReconJoint(Session, Tower, JointList, karmaparams, bin_time, PCAParams, joint_file_name);
%
%  Results = sessTestPCAReconKARMAJoint(Session, Tower, JointList)
%
...
if(length(recs) == 1)
   disp('There is 1 recording')
    ...
    PCAJoint(PCAParams.nDim+1:end) = [];
    PCAParams.U = PCAParams.U(1:end,1:PCAParams.nDec);
    PCAJoint = PCAJoint(1:PCAParams.nDec);
    jointdata = reconPCAJoint(PCAJoint,PCAParams);
    for iJoint = 1:size(jointdata,1)
        jointdata_tmp{iJoint}(:,1) = tmpJoint{iJoint}(1,:)';
        jointdata_tmp{iJoint}(:,2) = medfilt1(jointdata(iJoint,:)',15);
    end
    jointdata = jointdata_tmp;
     
    for iJoint = 1:nJoint
        original_jointdata{iJoint}(:,1) = Joint{ind(iJoint)}(1,:)';
        original_jointdata{iJoint}(:,2) = medfilt1(Joint{ind(iJoint)}(2,:)',15);
    end  
    
    if(0)
    for iJoint = 1:nJoint
        a = corrcoef(original_jointdata{iJoint}(:,2),jointdata{iJoint}(:,2));
        tmp(iJoint) = a(2)
    end
    pause
    end
...
[dum1, original_jointPos, dum2] = bin(pk,  binwidth, original_jointdata);
...
original_jointPos = original_jointPos(idx,:);

if(bin_time < 0)
    X = jointPos;
    jointPosTrain = X(1:floor(end/2),:);
    jointPosTest = X(floor(end/2)+1:end,:);
    X_orig = original_jointPos;
    original_jointPosTrain = X_orig(1:floor(end/2),:);
    original_jointPosTest = X_orig(floor(end/2)+1:end,:);
...
for iLoop = 1:loops
    ...
    original_jointTrain = [original_jointPosTrain];
    original_jointTest = [original_jointPosTest];
    ...
    jointTest = jointTest';
    original_jointTest = original_jointTest';
    nJoint = size(jointTest,1);
    KARMACorrCoef = zeros(1,nJoint);
    for iJoint = 1:nJoint
        a = corrcoef(jointTest(iJoint,:),jointPred(iJoint,:));
        reduced_KARMACorrCoef(iJoint) = a(2);
        a = corrcoef(original_jointTest(iJoint,:),jointPred(iJoint,:));
        KARMACorrCoef(iJoint) = a(2);
    end
    
    Results.Joint(1+(iLoop-1)*2).CorrCoef = KARMACorrCoef;
    Results.Joint(1+(iLoop-1)*2).ReducedCorrCoef = reduced_KARMACorrCoef;
    ...
    original_jointTrain = [original_jointPosTest];
    original_jointTest = [original_jointPosTrain];
    ...
    Results.Joint(iLoop*2).CorrCoef = KARMACorrCoef;
    Results.Joint(iLoop*2).ReducedCorrCoef = reduced_KARMACorrCoef;
