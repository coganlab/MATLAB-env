...
%{{'DelReachSaccade','DelSaccadeTouch','DelReachFix'}}

middle_value = [290, 295];
fk = 200; tapers = [.3,5];
CondParams(1,1).Name = ['ChoiceGenePlay']; 
...
CondParams(2,1).sort{1} = {'MovementDirection',Dirs(2)};
CondParams(2,1).sort{2} = {'CorrectChoice',1};
CondParams(2,1).sort{3} = {'HighRewardValue',320};

CondParams(2,2).Task = CondParams(2,1).Task;
CondParams(2,2).conds = {[Dirs(1)]};
CondParams(2,2).condstype = {{'Movement'}};
CondParams(2,2).sort{1} = {'MovementDirection',Dirs(2)};
CondParams(2,2).sort{2} = {'CorrectChoice',1};
CondParams(2,2).sort{3} = {'HighRewardValue',middle_value};

CondParams(2,3).Task = CondParams(2,1).Task;
CondParams(2,3).conds = {[Dirs(1)]};
CondParams(2,3).condstype = {{'Movement'}};
CondParams(2,3).sort{1} = {'MovementDirection',Dirs(2)};
CondParams(2,3).sort{2} = {'CorrectChoice',1};
