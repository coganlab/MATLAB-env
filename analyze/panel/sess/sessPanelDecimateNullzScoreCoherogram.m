function Data = sessPanelDecimateNullzScoreCoherogram(Sess, CondParams, AnalParams)
%
%   Data = sessPanelNullzScoreCoherogram(Sess,CondParams,AnalParams)
%
%   Decimates the firing rate with probability AnalParams.p
    ...
    switch Type
	    ...
	    Coh = CrossSpec./sqrt(SLfp.*SSpike);
            ...
            for iWin = 1:nwin
                ...
                %DPerm = bsxfun(@rdivide,DPerm,den);
                for iF = 1:size(D,2)
                    pCoh(iWin,iF) = length(find(DPerm(:,iF)>D(iWin,iF)))./nPerm;
                end
            end
