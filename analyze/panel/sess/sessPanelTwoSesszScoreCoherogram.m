function [Data, Raw] = sessPanelTwoSesszScoreCoherogram(Sess1, Sess2, CondParams, AnalParams)
%
%   [Data, Raw] = sessPanelzScoreCoherogram(Sess1,Sess2,CondParams,AnalParams)
%
% creates a z score for comparing to different SF or FF sessions
...
if(isfield(CondParams,'Diff'))
    ...
    else DiffCond = CondParams.Diff.conds;
    end
...
if isstruct(Sess2{1})
    ...
    disp('Loading data');
    All_Trials2 = sessTrials(Sess2);
end

Cond_Trials = Params2Trials(All_Trials1,CondParams);
DiffCond_Trials = Params2Trials(All_Trials2,DiffCondParams);
...
if length(Trials{1}) > 3 && length(DiffTrials{1}) > 3 && ...
    ...
    switch Type
        case 'FieldField'
            ...
            Ch = sessElectrode(Sess1);
            Contact = sessContact(Sess1);
            MonkeyDir = sessMonkeyDir(Sess1);
            ...
            % Diff
                ...
                Sys2 = Sys{2};
                if iscell(Sys2); Sys2 = Sys2{1}; end
                Ch = sessElectrode(Sess2);
                Contact = sessContact(Sess2);
                MonkeyDir = sessMonkeyDir(Sess2);
        ...
        case {'SpikeField','MultiunitField'}
            ...
            elseif diff(Diffbn)==diff(bn)
                ...
                DiffLfp = trialLfp(DiffTrials{1}, Sys{2}, Ch(2), Contact(2), DiffField, ...
                    [Diffbn(1)-N./2*1e3,Diffbn(2)+N./2*1e3], MonkeyDir);
                thresh = 6*std(DiffLfp(:)); e = max(abs(DiffLfp'));
                n = sum(DiffSpike,2)';
                ind = find(e<thresh & n>0);
