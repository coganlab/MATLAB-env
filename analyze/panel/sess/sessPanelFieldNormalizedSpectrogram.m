function Data = sessPanelFieldNormalizedSpectrogram(Sess,CondParams, AnalParams)
%
%   Data = sessFieldNormalizedSpectrogram(Sess,CondParams,AnalParams)
%
...
NormTask = '';
NormCond = {[]};
Normbn = [-tapers(1)*sampling_rate,0];
NormField = 'TargsOn';
...
if length(Trials{1}) > 3
    ...
    if isempty(find(isnan([Trials{1}.(Field)]), 1))
        ...
        t = bn(1):dn*1e3:bn(2);
        numtrials = 0;
        NormSpec = ones(length(t),diff(nfk),'single');
    end
else
...
if isstruct(FieldSession{1})
  FieldSession{1} = Trials{1}(1).Day;
  Data.Session = FieldSession;
end
