function Data = sessPanelField1zScoreSpectrogram(Sess,CondParams, AnalParams)
%
%   Data = sessField1zScoreSpectrogram(Sess,CondParams,AnalParams)
%
...
if(isfield(CondParams,'Norm'))
    if(isfield(CondParams.Norm,'Task'))
        NormTask = CondParams.Norm.Task;
    end
    if(isfield(CondParams.Norm,'Cond'))
        NormCond = CondParams.Norm.Cond;
    end
end
...
if length(Trials{1}) > 3
    ...
    if diff(Normbn) == N*sampling_rate;
        ...
        Lfp = trialLfp(NormTrials{1}, Sys, Ch, Contact, NormField,Normbn);
        thresh = 6*std(Lfp(:));
        e = max(abs(Lfp'));
        ind = find(e<thresh);
        Lfp = Lfp(ind,:);
    ...
    elseif diff(Normbn)==diff(bn)
        NormLfp = trialLfp(NormTrials{1}, Sys, Ch, Contact, NormField,...
            [Normbn(1)-N/2*sampling_rate,Normbn(2)+N/2*sampling_rate]);
