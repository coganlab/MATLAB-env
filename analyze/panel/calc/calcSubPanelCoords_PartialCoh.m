function PanelCoords = calcSubPanelCoords_PartialCoh(AnalParams, PlotParams, PanelCoords)
%
...
SubM = size(AnalParams,1) * 3; % Number of rows = Number of plot-types
SubN = size(AnalParams,2) * 2; % Number of columns = Number of alignment times
...
TotalTime = 0;
for iSubN = 1:SubN
  TotalTime = TotalTime + diff(AnalParams(1,ceil(iSubN/2)).bn);
end
PropTime = zeros(1,SubN);
for iSubN = 1:SubN
  PropTime(iSubN) = diff(AnalParams(1,ceil(iSubN/2)).bn)./TotalTime;
end
