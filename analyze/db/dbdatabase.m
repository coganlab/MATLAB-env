function [Trials] = dbdatabase(Days,MonkeyDir)
            ...
            for r = 1:nr
                ...
                if isfile([MonkeyDir '/' day '/' recs{r} '/rec' recs{r} '.Events.mat'])
                    ...
                    for t = 1:ntr
                        ...
                        if isfield(Events,'StartAq')
                            Trials(ind).StartAq = Events.StartAq(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'TargsOn')
                            Trials(ind).TargsOn = Events.TargsOn(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'Go')
                            Trials(ind).Go = Events.Go(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'TargAq')
                            Trials(ind).TargAq = Events.TargAq(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'ReachStart')
                            Trials(ind).ReachStart = Events.ReachStart(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'ReachStop')
                            Trials(ind).ReachStop = Events.ReachStop(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'SaccStart')
                            Trials(ind).SaccStart = Events.SaccStart(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        if isfield(Events,'SaccStop')
                            Trials(ind).SaccStop = Events.SaccStop(trials(t)) - ...
                                Events.StartOn(trials(t));
                        end
                        % ------------ Human Experiments Columbia--------
                        if isfield(Events,'StimOn')
                            Trials(ind).StimOn = Events.StimOn(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'StimID')
                            Trials(ind).StimID = Events.StimID(trials(t));
                        end
                        if isfield(Events,'StimOff')
                            Trials(ind).StimOff = Events.StimOff(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'Stim2On')
                            Trials(ind).Stim2On = Events.Stim2On(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'Stim2ID')
                            Trials(ind).Stim2ID = Events.Stim2ID(trials(t));
                        end
                        if isfield(Events,'Stim2Off')
                            Trials(ind).Stim2Off = Events.Stim2Off(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'Response')
                            Trials(ind).Response = Events.Response(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'ResponseID')
                            Trials(ind).ResponseID = Events.ResponseID(trials(t));
                        end
                        if isfield(Events,'Response2')
                            Trials(ind).Response2 = Events.Response2(trials(t)) - ...
                                Events.StartOn(trials(t)); 
                        end
                        if isfield(Events,'Response2ID')
                            Trials(ind).Response2ID = Events.Response2ID(trials(t));
                        end
                        if isfield(Events,'RewardID')
                            Trials(ind).RewardID = Events.RewardID(trials(t)); 
                        end
                        % ------------------------------------        
                        ...
                        if isfield(Events,'TargetDimming')
                            Trials(ind).TargetDimming = Events.TargetDimming(trials(t));
                        else
                            Trials(ind).TargetDimming = nan;
                        end
                        ...
                        if isfield(Events, 'PulseStarts')
                            Trials(ind).PulseStarts = Events.PulseStarts(trials(t));
                            Trials(ind).PulseStops = Events.PulseStops(trials(t));
                            Trials(ind).StartStim = Events.StartStim(trials(t));
                        end
                        
						% Continuous Target Information (Task Controller >v7.0)
                        if isfield(Events,'ContinuousTargets'); Trials(ind).ContinuousTargets = Events.ContinuousTargets(trials(t)); end
						if isfield(Events,'ContinuousTarget1'); Trials(ind).ContinuousTarget1 = Events.ContinuousTarget1(trials(t)); end % target 1 is always circle, target 2 is always triangle
                        if isfield(Events,'ContinuousTarget2'); Trials(ind).ContinuousTarget2 = Events.ContinuousTarget2(trials(t)); end % (TargetAngle = ContinuousTarget1./18 .*2 .*pi)
                        try
                            if isfield(Events,'ContinuousTarget1Cood'); Trials(ind).ContinuousTarget1Cood = Events.ContinuousTarget1Cood(trials(t),:); end
                            if isfield(Events,'ContinuousTarget2Cood'); Trials(ind).ContinuousTarget2Cood = Events.ContinuousTarget2Cood(trials(t),:); end
                        catch
                            if isfield(Events,'ContinuousTarget1Cood'); Trials(ind).ContinuousTarget1Cood = Events.ContinuousTarget1Cood(trials(t)); end
                            if isfield(Events,'ContinuousTarget2Cood'); Trials(ind).ContinuousTarget2Cood = Events.ContinuousTarget2Cood(trials(t)); end
                        end
						if isfield(Events,'ContinuousRadius1'); Trials(ind).ContinuousRadius1 = Events.ContinuousRadius1(trials(t)); end
						if isfield(Events,'ContinuousRadius2'); Trials(ind).ContinuousRadius2 = Events.ContinuousRadius2(trials(t)); end
						
						% Continuous Target Choice Information
						if isfield(Events,'MovementChoiceContinuous'); Trials(ind).MovementChoiceContinuous = Events.MovementChoiceContinuous(trials(t)); end
						if isfield(Events,'MovementErrorContinuous'); Trials(:,ind).MovementErrorContinuous = Events.MovementErrorContinuous(:,trials(t)); end
						if isfield(Events,'ChoseCircleContinuous'); Trials(ind).ChoseCircleContinuous = Events.ChoseCircleContinuous(trials(t)); end
						if isfield(Events,'ChoseTriangleContinuous'); Trials(ind).ChoseTriangleContinuous = Events.ChoseTriangleContinuous(trials(t)); end
						if isfield(Events,'ChoseHighRewContinuous'); Trials(ind).ChoseHighRewContinuous = Events.ChoseHighRewContinuous(trials(t)); end
						if isfield(Events,'ChoseLowRewContinuous'); Trials(ind).ChoseLowRewContinuous = Events.ChoseLowRewContinuous(trials(t)); end
                        ...
                        if isfield(Events,'RewardReceived')
                            Trials(ind).RewardReceived = Events.RewardReceived(trials(t));
                        else
                            Trials(ind).RewardReceived = nan;
                        end
                        ...
                        if isfield(Events,'StimType')
                            Trials(ind).StimType = Events.StimType(trials(t),:);
                        else
                            Trials(ind).StimType = nan;
                        end
                        ...
                        if isfield(Events,'ReachChoiceContinuousLocation')
                            Trials(ind).ReachChoiceContinuousLocation = Events.ReachChoiceContinuousLocation(trials(t),:);
                        else
                            Trials(ind).ReachChoiceContinuousLocation = nan;
                        end
