function Session = dbS(Sys,Task,N,conds, ControlTask, ControlField, ControlType, ControlQuantity, ControlLevel)
%
%   Session = dbS(Sys,Task,N,conds, ControlTask, ControlField, ControlType, ControlQuantity, ControlLevel)
%
%       Spike recordings
%
...
if nargin > 2
    NumTrialsFlag = 1;
    load([MONKEYDIR '/mat/Spike/Spike_NumTrials.mat']);
end
if length(conds{1})>0
...
Type = Sys{1};
disp([num2str(length(Session)) ' ' Type ' spike recordings']);
...
if nargin > 1
    ...
    if ~isempty(Task)
        ...
        disp([num2str(length(Session)) ' ' TaskString ' ' Type ' Spike sessions']);
    else
        disp([num2str(length(Session)) ' ' Type ' Spike sessions']);
    end
    ...
    if ~isempty(ControlTask)
        ...
        ind = [];
        for iSess = 1:length(Session)
            load([MONKEYDIR '/mat/Spike/Spike_ControlTuning.' num2str(SessNums(iSess)) '.mat']);
        ...
        eval(cmd);
        Session = Session(Ind);
        disp([num2str(length(Session)) ' ' ControlField ' Tuned' TaskString ' ' Type ' Spike sessions']);
    end
end
