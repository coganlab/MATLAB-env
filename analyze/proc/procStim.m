function procStim(day,rec)
%
%   procStim(day,rec)
%   altered version of procDisplay for stimraw.dat
%
...
for iNum = num(1):num(2)
    % disp(num2str(iNum))
    recs{iNum}
    fid = fopen([MONKEYDIR '/' day '/' recs{iNum} '/rec' recs{iNum} '.stimraw.dat']);
    ...
    if size(events,1)>1
        ...
        if max(levels) < midlevel
            disp('No stim state changes')
             fid = fopen([MONKEYDIR '/' day '/' recs{iNum}  '/rec' recs{iNum} '.stimraw.txt'],'w');
             fprintf(fid, ' ');
             fclose(fid);
        else
            h = 10;
...
%             a = find(fd(1:end-1) == 0 & fd(2:end) == 1) + 1;
            ...
            if size(A,2)>0 & size(TrialStarts,1)>1
                ...
                for iTr = 1:length(TrialStarts)-1
                    ...
                    %pause
                    if length(ind)>50
                        disp(['Warning: ' num2str(length(ind)) ' stim state changes on ' num2str(iTr)]);
                        pause
                    end
