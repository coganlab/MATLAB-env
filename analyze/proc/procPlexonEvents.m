function Data = procPlexonEvents(day, rec)
%
...
for iRec = num(1):num(2)
    ...
    fid = fopen(['rec' prenum '.LaserV.raw.dat']);
    laserData = fread(fid, 1e7, 'short');
    fclose(fid);
    
    pulseTimes = find(laserData(1:end-1) <= 20 & laserData(2:end) > 20);
    ...
    pulseEnds = find(laserData(1:end-1) > 20 & laserData(2:end) <=20);
