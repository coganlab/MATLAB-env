    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                ...
                Events.LEDBoard(trial) = evs(Ind_StartOn-1,2)-30;
                Events.EyeTarget(trial) = evs(Ind_StartOn-2,2)-10+1;
                Events.Reward2T(trial) = evs(Ind_StartOn-3,2)-30+1;
                Events.Choice(trial) = evs(Ind_StartOn-4,2) - 10+1;
                Events.RewardMag(trial) = evs(Ind_StartOn-5,2) - 30+1;
                Events.Reward(trial) = evs(Ind_StartOn-6,2) - 10+1;
                Events.Target(trial) = evs(Ind_StartOn-7,2)-30+1;
                Events.StartHand(trial) = evs(Ind_StartOn-8,2)-10+1;
                Events.StartEye(trial) = evs(Ind_StartOn-9,2)-30+1;
                Events.HandCode(trial) = evs(Ind_StartOn-10,2)-10;
                Events.Joystick(trial) = evs(Ind_StartOn-11,2)-50;
                Events.TaskCode(trial) = evs(Ind_StartOn-12,2)-10;
                Events.TaskCode(trial)

                if length(display_evs) & (Events.LEDBoard(trial) == 0)
