...
try
    ...
    for i = 1:length(lv_st);
        ...
        lv_events(i,1) = z1*60*60*1000 + z2*60*1000 + z3*1000;  % Convert to milliseconds from start of day
    end
...
try
    ...
    for i = 1:length(lv_st);
        ...
        while length(z)<6; z = ['0',z]; end
        z1 = str2num(z(1:2)); z2 = str2num(z(3:4)); z3 = str2num(z(5:6));
        lv_target(i,1) = z1*60*60*1000 + z2*60*1000 + z3*1000;   % Convert to milliseconds from start of day
    end
    ...
    if ~isempty(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if ~isempty(display_events)
                ...
                display_evs=[];
                display(['WARNING: No display events on trial ' num2str(tr) ' in recording ' num2str(iRec)])
                pause
            end
...
%                     end
                ...
                if find(evs(Ind_StartOn:end,2)==15)
                    ...
                    % Is this a choice task not a Reach and Saccade Task
                    % and a successful task
                    if(Events.Choice(trial) == 2 && length(Ind_Success) > 0 && Events.TaskCode(trial) ~= 13)
                        % Problem detecting event code 
                        disp('Warning EffInstOn code not detected');
                        pause
                        Events.Go(trial)
                        Events.TargsOn(trial)
                        tmp = min(display_evs(find(display_evs(:,2)>Events.TargsOn(trial)),2))
                        if(size(tmp) > 0)
                            Events.EffInstOn(trial) = min(display_evs(find(display_evs(:,2)>Events.TargsOn(trial)),2));
                        else
                            disp('Warning: No display sensor event')
                            display_evs(:,2)
                            pause
