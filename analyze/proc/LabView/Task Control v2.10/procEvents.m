    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                ...
                Events.Joystick(trial) = evs(Ind_StartOn-9,2)-30;
                ...
                if find(evs(Ind_StartOn:end,2)==10);
                    ...
                    Events.TargetRet(trial) = evs(index(1)+Ind_StartOn,3);
                end
                ...
                if isReachTrial(Events.TaskCode(trial)) && Events.Success(trial)
