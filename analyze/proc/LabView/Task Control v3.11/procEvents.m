...
for iRec = num(1):num(2)
    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn && Ind_StartOn~=size(evs,1) && ~isempty(display_evs)
...
%                 if find(evs(:,2)==3) & ~isempty(display_evs)
%                     index = find(evs(:,2)==3);
...
%                     end
% 
%                 elseif Events.TaskCode(trial) == 21%% TOA task
