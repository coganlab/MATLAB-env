...
for iRec = num(1):num(2)
    ...
    if length(events)
        ...
        Events.RewardBlock = zeros(ntr,1);
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                Events.Separable4T(trial) = evs(Ind_StartOn-1,2) - 10+1; %Added for v3.20
                Events.Trial(trial) = tr; 
                temp = evs(Ind_StartOn-2,2)-30; %Added for v3.17
                Events.AdaptationFeedback(trial) = floor(temp/5);
                Events.AdaptationAction(trial) = mod(temp,5);
                Events.AdaptationPhase(trial) = evs(Ind_StartOn-3,2)-10;
                Events.LEDBoard(trial) = evs(Ind_StartOn-4,2)-30; %Added for v3.16
                Events.RewardConfig(trial) = evs(Ind_StartOn-5,2)-10+1;  %   Which configuration of alternatives
                Events.Choice(trial) = evs(Ind_StartOn-6,2) - 30+1;       %  Free or instructed choice
                Events.RewardBlock(trial) = evs(Ind_StartOn-7,2) - 10+1;  %  Which mag or prob block encoded - need printout
                Events.RewardTask(trial) = evs(Ind_StartOn-8,2) - 30+1;   %  Which type of reward task - simple, 2t, 4t etc
                
                eyetarget = (evs(Ind_StartOn-10,2) - 30) * 10 + evs(Ind_StartOn-9,2) - 10 + 1;
                Events.EyeTarget(trial) = eyetarget; %Added for v3.15
                handtarget = (evs(Ind_StartOn-12,2) - 30) * 10 + evs(Ind_StartOn-11,2) - 10 + 1;
                Events.Target(trial) = handtarget;
                
                Events.StartHand(trial) = evs(Ind_StartOn-13,2)-10+1;
                Events.StartEye(trial) = evs(Ind_StartOn-14,2)-30+1;
                Events.HandCode(trial) = evs(Ind_StartOn-15,2)-10;
                Events.Joystick(trial) = evs(Ind_StartOn-16,2)-50;
                Events.TaskCode(trial) = evs(Ind_StartOn-17,2)-10;
...
%---------------End specific to trial type---------------------------
                ...
                if find(evs(Ind_StartOn:end,2)==10)  %Only for Memory trials
                    if length(display_evs) 
                        ...
                        Events.TargetRet(trial) = min(display_evs(find(display_evs(:,2)>evs(index(1)+Ind_StartOn-1,3)),2));
                        display_evs = display_evs(2:end,:);
                    else
                        index = find(evs(Ind_StartOn:end,2)==10);
		  ...
		  %pause
            ...
            %pause
                ...
                if (isReachTrial(Events.TaskCode(trial)) | Events.ReachGo(trial)>0) & Events.Success(trial) & ~Events.Joystick(trial)
