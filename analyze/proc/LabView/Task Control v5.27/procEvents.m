...
%                 end
                ...
                try
                    ...
                    if lvFirstTime
                        %disp('In lvFirstTime')
                        [x targetlv_trial] = min(abs(st-lv_target(:,1)));
                        targetlv_trial = targetlv_trial;
%			pause
%                         lv_trial
                        lvFirstTime = 0;
                           %pause
                        ...
                        %disp('Not First Time');
                        targetlv_trial = targetlv_trial+1;
%                         lv_trial
%                         pause
                    end
                    Events.EyeTargetLocation(trial,:) = lv_target(targetlv_trial,4:5);
                    Events.HandTargetLocation(trial,:) = lv_target(targetlv_trial,2:3);
                    Events.TargetDelta(trial) = st - lv_target(targetlv_trial,1);
                    Events.Targetlvtrial(trial) = targetlv_trial;
