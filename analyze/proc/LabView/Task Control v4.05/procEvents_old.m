...
%                 Events.Delta(trial) = st - lv_events(lv_trial,1);
                ...
                if find(evs(Ind_StartOn:end,2)==4)
                    ...
                    if ~isempty(display_evs) && (Events.TaskCode(trial) == 11) %detects the LED change during DS adaptation
                ...
                if find(evs(Ind_StartOn:end,2)==9);
                    if ~isempty(display_evs) && Events.LEDBoard(trial)==1
                        index = find(evs(:,2)==9);
