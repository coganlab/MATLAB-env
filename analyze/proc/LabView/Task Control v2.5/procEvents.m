...
for iRec = num(1):num(2)
	...
	Events.Trial = zeros(ntr,1);	%  Trial number in ev.txt
		...
		Events.Trial(trial) = tr;
                ...
                Events.Target(trial) = evs(Ind_StartOn(1)-1,2)-30+1;
                Events.StartHand(trial) = evs(Ind_StartOn(1)-2,2)-10+1;
                Events.StartEye(trial) = evs(Ind_StartOn(1)-3,2)-30+1;
                Events.HandCode(trial) = evs(Ind_StartOn(1)-4,2)-10;
                Events.Joystick(trial) = evs(Ind_StartOn(1)-5,2)-30;
                Events.TaskCode(trial) = evs(Ind_StartOn(1)-6,2)-10;

                Events.StartOn(trial) = evs(Ind_StartOn(1),3);
