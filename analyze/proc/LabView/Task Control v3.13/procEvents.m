    ...
    if length(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn & Ind_StartOn~=size(evs,1)
                ...
                Events.EyeTarget(trial) = evs(Ind_StartOn-1,2)-10+1;
                Events.Reward2T(trial) = evs(Ind_StartOn-2,2)-30+1;
                Events.Choice(trial) = evs(Ind_StartOn-3,2) - 10+1;
                Events.RewardMag(trial) = evs(Ind_StartOn-4,2) - 30+1;
                Events.Reward(trial) = evs(Ind_StartOn-5,2) - 10+1;
                Events.Target(trial) = evs(Ind_StartOn-6,2)-30+1;
                Events.StartHand(trial) = evs(Ind_StartOn-7,2)-10+1;
                Events.StartEye(trial) = evs(Ind_StartOn-8,2)-30+1;
                Events.HandCode(trial) = evs(Ind_StartOn-9,2)-10;
                Events.Joystick(trial) = evs(Ind_StartOn-10,2)-50;
                Events.TaskCode(trial) = evs(Ind_StartOn-11,2)-10;
...
%                 end
% 
%                 if find(evs(:,2)==6) & length(display_evs);
%                     index = find(evs(:,2)==6);
...
%                     if find(evs(Ind_StartOn:end,2)==12) & length(display_evs)
...
%                     end
% 
%                     if find(evs(Ind_StartOn:end,2)==11) & length(display_evs)
...
%                     if find(evs(Ind_StartOn:end,2)==12) & isempty(find(evs(Ind_StartOn:end,2)==11)) & length(display_evs)
%                         %  When there is a Saccade Go but no Reach Go.  ie TOA = 0;
%                         index = find(evs(Ind_StartOn:end,2)==12);
%                         Events.SaccadeGo(trial) = min(display_evs(find(display_evs(:,2)>evs(index(1)+Ind_StartOn-1,3)),2));
%                         Events.ReachGo(trial) = Events.SaccadeGo(trial);
%                         display_evs = display_evs(2:end,:);
%                     elseif find(evs(Ind_StartOn:end,2)==12) & find(evs(Ind_StartOn:end,2)==11) & length(display_evs)
...
% 
%                 if find(evs(Ind_StartOn:end,2)==10)  & length(display_evs) %Only for Memory trials
...
%                 end
                ...
                if find(evs(Ind_StartOn:end,2)==12)
                    ...
                    Events.ReachGo(trial) = Events.SaccadeGo(trial);
                    if isnan(Events.TargsOn(trial))
                        Events.TargsOn(trial) = Events.SaccadeGo(trial);
                    end
                end
