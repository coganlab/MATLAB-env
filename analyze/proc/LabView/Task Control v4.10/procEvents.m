    ...
    if ~isempty(events)
        ...
        for tr = min(events(:,1))+1:max(events(:,1))-1  %  Skip first and
            ...
            if Ind_StartOn && Ind_StartOn~=size(evs,1)
                ...
                Events.HighReward(trial) = evs(Ind_StartOn-n+19,2) - 30+1;   %  Which is the high reward target
...
%                         vel(1:50) = vel(51);
%                         [s,sac] = max(vel);
%                         a = find(vel(max([1,sac-100]):sac) > 350);
%                         if isempty(a)
%                             a = find(vel(max([1,sac-100]):sac) > 300);
%                         end
%                         if isempty(a)
...
%                         end
%                         a = find(vel(sac:min([end,sac+100])) > 350);
%                         if isempty(a)
%                             a = find(vel(sac:min([end,sac+100])) > 300);
%                         end
%                         if isempty(a)
